...
Matrix-series lemmas precede one section with a shared checked-absorption
hypothesis. It derives the first-hit probability law, both integrability
results, and `solve_correct`. The final section proves that the automatic
checker is equivalent to full-domain absorption and gives `autoSolve_correct`.
-/
...
private lemma summable_powers_of_block {A : Matrix (Fin n) (Fin n) ℝ}
...
private lemma summable_weighted_powers_of_block {A : Matrix (Fin n) (Fin n) ℝ}
    {k : ℕ} (hk : 0 < k) (hA : ‖A ^ k‖ < 1) :
...
private lemma weighted_series_identity {A : Matrix (Fin n) (Fin n) ℝ}
...
private lemma hasSum_mulVec {f : ℕ → Matrix (Fin n) (Fin m) ℝ}
...
private lemma hasSum_mul {f : ℕ → Matrix (Fin n) (Fin n) ℝ}
...
private lemma cast_hit (r : ℕ) : (C.hit r).map (Rat.castHom ℝ) = Qr ^ r * Rr := by
  simp only [hit, Matrix.map_mul, Matrix.map_pow]

private lemma real_row_sum : (1 - Qr) *ᵥ (fun _ => (1 : ℝ)) = Rr *ᵥ (fun _ => 1) := by
...
private lemma real_hit_nonneg (r : ℕ) (i : Fin n) (a : Fin m) :
    0 ≤ (C.hit r i a : ℝ) := by exact_mod_cast C.hit_nonneg r i a

private lemma summable_joint_nonneg {f : ℕ → Fin m → ℝ} (hf : ∀ r a, 0 ≤ f r a)
...
/-! ## Consequences of a checked absorption bound -/

section CheckedAbsorption

variable {k : ℕ} {c : ℚ}
variable (h : C.admissible k c = true)

include h

section Summability

private lemma norm_pow_bound :
    ‖Qr ^ k‖ < 1 := by
...
theorem summable_weighted_powers :
...
theorem hasSum_firstHit (i : Fin n) :
...
private lemma summable_firstHit (i : Fin n) :
...
theorem firstHit_probability (i : Fin n) :
...
section RewardAndTime

private lemma series_solution
...
private lemma summable_reward_norm
    (i : Fin n) (f : Fin m → ℝ) :
...
private lemma integral_reward_series
    (i : Fin n) (f : Fin m → ℝ) :
...
private lemma hasSum_weighted_hits :
...
private lemma summable_time (i : Fin n) :
...
theorem integrable_time (i : Fin n) :
...
private lemma integral_time_series (i : Fin n) :
...
private lemma integral_reward_of_bellman (v : Fin n → ℚ)
    (hv : (1 - C.Q) *ᵥ v = C.R *ᵥ C.reward) (i : Fin n) :
    (∫ p : ℕ × Fin m, (C.reward p.2 : ℝ) ∂(Measure.sum fun p : ℕ × Fin m =>
      ENNReal.ofReal (C.hit p.1 i p.2 : ℝ) • Measure.dirac p)) = (v i : ℝ) := by
  rw [C.integral_reward_series h i (fun a => (C.reward a : ℝ)), ← Matrix.mulVec_mulVec]
  have hcast : Rr *ᵥ (fun a => (C.reward a : ℝ)) =
      fun j => ((C.R *ᵥ C.reward) j : ℝ) := by
    funext j
    exact ((Rat.castHom ℝ).map_mulVec C.R C.reward j).symm
  rw [hcast, C.series_solution h hv]

private lemma integral_time_of_bellman (t : Fin n → ℚ)
    (ht : (1 - C.Q) *ᵥ t = (fun _ => 1)) (i : Fin n) :
    (∫ p : ℕ × Fin m, ((p.1 + 1 : ℕ) : ℝ) ∂(Measure.sum fun p : ℕ × Fin m =>
      ENNReal.ofReal (C.hit p.1 i p.2 : ℝ) • Measure.dirac p)) = (t i : ℝ) := by
  rw [C.integral_time_series h i]
  simpa only [Rat.cast_one] using congrFun (C.series_solution h ht) i
...
theorem solve_correct :
  ...
  intro i
  dsimp only
  exact ⟨C.firstHit_probability h i,
    C.integrable_reward h i (fun a => (C.reward a : ℝ)), C.integrable_time h i,
    C.integral_reward_of_bellman h v ev i, C.integral_time_of_bellman h t et i⟩

end CheckedAbsorption
...
lemma firstHit_singleton (i : Fin n) (r : ℕ) (a : Fin m) :
...
/-! ## Automatic full-domain absorption -/

section AutomaticAbsorption
...
lemma autoCheck_det_ne_zero : (1 - C.Q).det ≠ 0 :=
  C.det_ne_zero h
...
theorem autoSolve_integrals (i : Fin n) (value duration : ℚ)
...
lemma autoSolve_ne_singular : C.autoSolve ≠ .error .singular := by
