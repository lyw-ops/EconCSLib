...
The dependent `Fin k` product uses a private prefix accumulator, so the public
surface contains only the completed product operation rather than its recursion
machinery.
...
def complete {k : ℕ} {X : Fin k → Type uα}
    {count : ℕ} (acc : FinPrefix X count)
    (hcount : count = k) :
    (i : Fin k) → X i :=
...
theorem fintypePi_map {I : Type*} [Fintype I] [LinearOrder I]
...
complete tuple. -/
theorem fintypePi_pure {I : Type*} [Fintype I] [LinearOrder I]
...
theorem finPiFrom_apply_eq_of_hasPositiveAtom_of_lt
    {k : ℕ} {X : Fin k → Type uα}
    (laws : (i : Fin k) → FiniteLaw (X i)) :
    ∀ (remaining count : ℕ)
      (htotal : count + remaining = k)
...
private theorem sum_weight_mul_indicator
...
private theorem mass_map_pair_left
...
private theorem finPiFrom_mass_of_prefix
    {k : ℕ} {X : Fin k → Type uα}
    [∀ i, DecidableEq (X i)]
...
private theorem finPiMassFrom_eq_prod
    {k : ℕ} {X : Fin k → Type uα}
    [∀ i, DecidableEq (X i)]
    (laws : (i : Fin k) → FiniteLaw (X i))
    (tuple : (i : Fin k) → X i) :
...
private theorem expectRat_finPiFrom_lt
    {k : ℕ} {X : Fin k → Type uα}
    (laws : (i : Fin k) → FiniteLaw (X i))
    (remaining count : ℕ) (htotal : count + remaining = k)
...
private theorem expectRat_finPiFrom_ge
    {k : ℕ} {X : Fin k → Type uα}
    (laws : (i : Fin k) → FiniteLaw (X i))
    (remaining count : ℕ) (htotal : count + remaining = k)
