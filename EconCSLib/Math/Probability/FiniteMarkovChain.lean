...
Transition rows lead to the `run_expect` specification of finite execution.
A supplied absorption bound supports exact Bellman solving. Survival lemmas
then establish finite reachability completeness and the automatic checker.
Routine lemmas support these main theorems without additional model fields.
-/
...
lemma expectRat_row (i : Fin n) (f : (Fin n ⊕ Fin m) → ℚ) :
    (C.row i).expectRat f =
...
lemma Q_nonneg (i j : Fin n) : 0 ≤ C.Q i j := (C.transient i j).property

/-- Nonnegative entries of the terminal block. -/
lemma R_nonneg (i : Fin n) (a : Fin m) : 0 ≤ C.R i a := (C.terminal i a).property

/-- Rational row normalization is inherited from the actual probability data. -/
lemma row_sum (i : Fin n) : (∑ j, C.Q i j) + (∑ a, C.R i a) = 1 := by
  dsimp [Q, R]
  exact_mod_cast C.normalized i

end TransitionRows

/-! ## Censored finite execution -/
...
lemma pow_nonneg (r : ℕ) (i j : Fin n) : 0 ≤ (C.Q ^ r) i j :=
  Matrix.pow_apply_nonneg C.Q_nonneg r i j

/-- Every first-hit coefficient is nonnegative. -/
lemma hit_nonneg (r : ℕ) (i : Fin n) (a : Fin m) : 0 ≤ C.hit r i a :=
  Finset.sum_nonneg fun j _ => mul_nonneg (C.pow_nonneg r i j) (C.R_nonneg j a)

private lemma observe_delay (active : Fin n → ℚ) (finished : ℕ → Fin m → ℚ) :
    (Sum.elim active (fun x => finished x.1 x.2)) ∘ delay =
...
private lemma transient_step (horizon : ℕ) (i : Fin n) (active : Fin n → ℚ) :
    (∑ j, C.Q i j * (C.Q ^ horizon *ᵥ active) j) =
      (C.Q ^ (horizon + 1) *ᵥ active) i := by
  change (C.Q *ᵥ (C.Q ^ horizon *ᵥ active)) i = _
  rw [Matrix.mulVec_mulVec, pow_succ']

private lemma firstHit_step (horizon : ℕ) (i : Fin n) (finished : ℕ → Fin m → ℚ) :
    (∑ j, C.Q i j *
      ∑ r ∈ Finset.range horizon, (C.hit r *ᵥ finished (r + 1)) j) =
...
theorem run_expect (horizon : ℕ) (i : Fin n)
  ...
  | succ horizon ih =>
    ...
    rw [C.transient_step horizon i active, C.firstHit_step horizon i finished]
...
lemma run_survival (horizon : ℕ) (i : Fin n) :
...
/-! ## Checking a supplied absorption bound -/

section AbsorptionBound
...
lemma solveLinear_spec (A : Matrix (Fin n) (Fin n) ℚ) (b : Fin n → ℚ)
    (hA : A.det ≠ 0) :
...
lemma solveLinear_unique (A : Matrix (Fin n) (Fin n) ℚ) (hA : A.det ≠ 0)
...
lemma survival_nonneg (horizon : ℕ) (i : Fin n) : 0 ≤ C.survival horizon i :=
...
lemma survival_le_one (horizon : ℕ) (i : Fin n) : C.survival horizon i ≤ 1 := by
...
lemma survival_antitone (i : Fin n) : Antitone (fun horizon => C.survival horizon i) := by
...
private lemma survival_deficit (horizon : ℕ) (i : Fin n) :
...
lemma survival_succ_lt_one_iff (horizon : ℕ) (i : Fin n) :
...
private lemma mem_finishWithin (horizon : ℕ) (i : Fin n) :
    i ∈ C.finishWithin horizon ↔ C.survival horizon i < 1 := by
  simp [finishWithin]

private lemma finishWithin_mono : Monotone C.finishWithin := by
  intro h k hle i hi
  rw [C.mem_finishWithin] at hi ⊢
  exact (C.survival_antitone i hle).trans_lt hi

private lemma finishWithin_step {h k : ℕ}
...
private lemma finishWithin_stable (horizon : ℕ) (hle : n ≤ horizon) :
...
lemma survival_le_autoBound (i : Fin n) : C.survival (n + 1) i ≤ (C.autoBound : ℚ) := by
...
lemma autoBound_bounds : 0 ≤ (C.autoBound : ℚ) ∧ C.autoBound ≤ 1 := by
...
lemma autoSolve_of_rejected (h : C.autoCheck = false) :
