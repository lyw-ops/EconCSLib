...
namespace Examples.ExtensiveGame.FiniteMarkovChain.Chain
...
/-- The original geometric first-hit law has mean reward seven and duration two,
now obtained from the automatic solver without a caller-supplied block bound. -/
theorem geometric_autoExpectations :
    let C := Regression.geometric
    let μ := Measure.sum fun p : ℕ × Fin 1 =>
      ENNReal.ofReal (C.hit p.1 0 p.2 : ℝ) • Measure.dirac p
    (∫ p : ℕ × Fin 1, (C.reward p.2 : ℝ) ∂μ) = 7 ∧
      (∫ p : ℕ × Fin 1, ((p.1 + 1 : ℕ) : ℝ) ∂μ) = 2 := by
  simpa using Regression.geometric.autoSolve_integrals Regression.automatic_checks.1
    0 7 2 Regression.geometric_autoResult

/-- Signed terminal rewards integrate to one under the original first-hit law;
the mean actual duration is two. -/
theorem twoRewards_autoExpectations :
    let C := Regression.twoRewards
    let μ := Measure.sum fun p : ℕ × Fin 2 =>
      ENNReal.ofReal (C.hit p.1 0 p.2 : ℝ) • Measure.dirac p
    (∫ p : ℕ × Fin 2, (C.reward p.2 : ℝ) ∂μ) = 1 ∧
      (∫ p : ℕ × Fin 2, ((p.1 + 1 : ℕ) : ℝ) ∂μ) = 2 := by
  simpa using Regression.twoRewards.autoSolve_integrals Regression.automatic_checks.2.1
    0 1 2 Regression.twoRewards_autoResult

/-- Automatic certification of delayed absorption yields the original reward
integral seven and actual duration integral three. -/
theorem delayed_autoExpectations :
    let C := Regression.delayed
    let μ := Measure.sum fun p : ℕ × Fin 1 =>
      ENNReal.ofReal (C.hit p.1 0 p.2 : ℝ) • Measure.dirac p
    (∫ p : ℕ × Fin 1, (C.reward p.2 : ℝ) ∂μ) = 7 ∧
      (∫ p : ℕ × Fin 1, ((p.1 + 1 : ℕ) : ℝ) ∂μ) = 3 := by
  simpa using Regression.delayed.autoSolve_integrals Regression.automatic_checks.2.2.1
    0 7 3 Regression.delayed_autoResult

/-- A rejected global input can still absorb from a particular start: state
zero terminates after one transition while state one has survival identically one. -/
theorem closedClass_absorption_boundary :
    Filter.Tendsto (fun horizon => (Regression.closedClass.survival horizon 0 : ℝ))
      Filter.atTop (nhds 0) ∧ ¬ Regression.closedClass.AbsorbsAll := by
  constructor
  · apply Filter.Tendsto.congr' (f₁ := fun _ : ℕ => (0 : ℝ)) _ tendsto_const_nhds
    filter_upwards [Filter.eventually_ge_atTop 1] with horizon hh
    obtain ⟨k, rfl⟩ := Nat.exists_eq_succ_of_ne_zero (by omega : horizon ≠ 0)
    simp [Regression.closedClass_good_start]
  · intro h
    have := Regression.closedClass.autoCheck_iff_absorbsAll.mpr h
    simp [Regression.automatic_rejections.1] at this

/-- An already terminal initial state has its original reward and zero duration;
its one-step law is absorbing and its time-zero law is a Dirac probability. -/
theorem terminal_semantics (a : Fin m) :
    C.step (.inr a) = FiniteLaw.pure (.inr a) ∧
    (∫ b : Fin m, (C.reward b : ℝ) ∂Measure.dirac a) = (C.reward a : ℝ) ∧
    (∫ _ : Fin m, (0 : ℝ) ∂Measure.dirac a) = 0 := by
  simp [step]

/-- A concrete consumer of the general semantic theorem: the state without
one-step absorption really has value seven and expected hitting time three. -/
theorem delayed_expectations :
    let C := Regression.delayed
    let μ := Measure.sum fun p : ℕ × Fin 1 =>
      ENNReal.ofReal (C.hit p.1 0 p.2 : ℝ) • Measure.dirac p
    (∫ p : ℕ × Fin 1, (C.reward p.2 : ℝ) ∂μ) = 7 ∧
      (∫ p : ℕ × Fin 1, ((p.1 + 1 : ℕ) : ℝ) ∂μ) = 3 := by
  have hb : Regression.delayed.admissible 2 (1 / 2) = true := by
    norm_num [admissible, Q, Regression.delayed, pow_two, Matrix.mul_apply, Fin.sum_univ_two]
  obtain ⟨v, t, hs, _, _, _, _, hsemantic⟩ := Regression.delayed.solve_correct hb
  have hv : v 0 = 7 ∧ t 0 = 3 := by
    have heq := congrArg (Except.map (fun vt => (vt.1 (.inl 0), vt.2 (.inl 0)))) hs
    rw [Regression.delayed_result] at heq
    simpa only [Except.map, Sum.elim_inl, Except.ok.injEq, Prod.mk.injEq, eq_comm] using heq
  obtain ⟨_, _, _, he, ht⟩ := hsemantic 0
  exact ⟨he.trans (by exact_mod_cast hv.1), ht.trans (by exact_mod_cast hv.2)⟩

end Examples.ExtensiveGame.FiniteMarkovChain.Chain
