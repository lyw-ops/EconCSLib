...
Opt-in fixed-chain prototypes. Transient and terminal states have explicit
`Fin` encodings. Nonnegative rational rows sum to one, and terminal states
...
before invoking that solver. Analytic correctness is proved downstream in
`FiniteMarkovChainSemantics`.
-/

namespace Examples.ExtensiveGame.FiniteMarkovChain
...
/-- A geometric self-loop followed by a terminal reward of seven. -/
def geometric : Chain 1 1 where
  transient := fun _ _ => 1 / 2
  terminal := fun _ _ => 1 / 2
  normalized := by intro i; norm_num
  reward := fun _ => 7

/-- Distinct signed terminal rewards, reached with equal eventual probabilities. -/
def twoRewards : Chain 1 2 where
  transient := fun _ _ => 1 / 2
  terminal := fun _ _ => 1 / 4
  normalized := by intro i; norm_num [Fin.sum_univ_two]
  reward := ![-3, 5]

/-- The first state cannot terminate in one step; two-step absorption is enough. -/
def delayed : Chain 2 1 where
  transient := !![0, 1; 0, 1 / 2]
  terminal := fun i _ => if i = 0 then 0 else 1 / 2
  normalized := by intro i; fin_cases i <;> norm_num [Fin.sum_univ_two]
  reward := fun _ => 7

/-- An unreachable-from-the-other-state closed class is still rejected: this
prototype checks the full supplied state domain, without restricting reachability. -/
def closedClass : Chain 2 1 where
  transient := !![0, 0; 0, 1]
  terminal := fun i _ => if i = 0 then 1 else 0
  normalized := by intro i; fin_cases i <;> norm_num [Fin.sum_univ_two]
  reward := fun _ => 7

/-- The geometric chain has value seven and expected duration two. -/
theorem geometric_result :
    (geometric.solve 1 (1 / 2)).map (fun vt => (vt.1 (.inl 0), vt.2 (.inl 0))) =
      .ok (7, 2) := by
  norm_num [Chain.solve, Chain.admissible, Chain.solveLinear, Chain.Q, Chain.R,
    geometric, twoRewards, delayed, closedClass, Matrix.det_fin_one, Matrix.det_fin_two,
    Matrix.cramer, Matrix.cramerMap, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
    pow_two, Matrix.mul_apply, Except.map]

/-- Terminal rewards are not replaced by a constant or by a zero fallback. -/
theorem twoRewards_result :
    (twoRewards.solve 1 (1 / 2)).map (fun vt => (vt.1 (.inl 0), vt.2 (.inl 0))) =
      .ok (1, 2) := by
  norm_num [Chain.solve, Chain.admissible, Chain.solveLinear, Chain.Q, Chain.R,
    geometric, twoRewards, delayed, closedClass, Matrix.det_fin_one, Matrix.det_fin_two,
    Matrix.cramer, Matrix.cramerMap, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
...
/-- Starting already terminal has the original payoff and hitting time zero. -/
theorem alreadyTerminal_result :
    (twoRewards.solve 1 (1 / 2)).map (fun vt => (vt.1 (.inr 1), vt.2 (.inr 1))) =
      .ok (5, 0) := by
  norm_num [Chain.solve, Chain.admissible, Chain.solveLinear, Chain.Q, Chain.R,
    geometric, twoRewards, delayed, closedClass, Matrix.det_fin_one, Matrix.det_fin_two,
    Matrix.cramer, Matrix.cramerMap, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
...
/-- A two-step certificate succeeds where every strict one-step bound fails. -/
theorem delayed_result :
    (delayed.solve 2 (1 / 2)).map (fun vt => (vt.1 (.inl 0), vt.2 (.inl 0))) =
      .ok (7, 3) := by
  norm_num [Chain.solve, Chain.admissible, Chain.solveLinear, Chain.Q, Chain.R,
    geometric, twoRewards, delayed, closedClass, Matrix.det_fin_one, Matrix.det_fin_two,
    Matrix.cramer, Matrix.cramerMap, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
...
/-- A false supplied bound is an explicit error. -/
theorem delayed_oneStep_rejected : delayed.solve 1 (1 / 2) = .error .absorptionBound  := by
  norm_num [Chain.solve, Chain.admissible, Chain.solveLinear, Chain.Q, Chain.R,
    geometric, twoRewards, delayed, closedClass, Matrix.det_fin_one, Matrix.det_fin_two,
    Matrix.cramer, Matrix.cramerMap, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
    pow_two, Matrix.mul_apply, Except.map]

/-- Finiteness alone does not imply absorption. -/
theorem closedClass_rejected : closedClass.solve 2 (3 / 4) = .error .absorptionBound  := by
  norm_num [Chain.solve, Chain.admissible, Chain.solveLinear, Chain.Q, Chain.R,
    geometric, twoRewards, delayed, closedClass, Matrix.det_fin_one, Matrix.det_fin_two,
    Matrix.cramer, Matrix.cramerMap, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
    pow_two, Matrix.mul_apply, Except.map]

/-- Singular linear systems never return a fictitious zero vector. -/
theorem singular_rejected :
    Chain.solveLinear (0 : Matrix (Fin 1) (Fin 1) ℚ) (fun _ => 1) = none := by
  norm_num [Chain.solve, Chain.admissible, Chain.solveLinear, Chain.Q, Chain.R,
    geometric, twoRewards, delayed, closedClass, Matrix.det_fin_one, Matrix.det_fin_two,
    Matrix.cramer, Matrix.cramerMap, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
    pow_two, Matrix.mul_apply, Except.map]

/-- Zero-length blocks and nonpositive absorption margins are rejected. -/
theorem invalidBounds_rejected :
    geometric.admissible 0 (1 / 2) = false ∧ geometric.admissible 1 1 = false := by
  norm_num [Chain.solve, Chain.admissible, Chain.solveLinear, Chain.Q, Chain.R,
    geometric, twoRewards, delayed, closedClass, Matrix.det_fin_one, Matrix.det_fin_two,
    Matrix.cramer, Matrix.cramerMap, Matrix.mulVec, dotProduct, Fin.sum_univ_two,
    pow_two, Matrix.mul_apply, Except.map]

/-- Distinct first-hit times retain their actual probabilities. -/
theorem firstHit_result :
    (geometric.run 3 0).expectRat (fun x => if x = .inr (2, 0) then 1 else 0) = 1 / 8 := by
  rw [geometric.run_firstHit 3 2 (by omega)]
  norm_num [Chain.hit, Chain.Q, Chain.R, geometric, pow_two, Matrix.mul_apply]

/-- info: (Except.ok (7, 2), Except.ok (1, 2), Except.ok (7, 3)) -/
#guard_msgs in
#eval ((geometric.solve 1 (1 / 2)).map (fun vt => (vt.1 (.inl 0), vt.2 (.inl 0))),
  (twoRewards.solve 1 (1 / 2)).map (fun vt => (vt.1 (.inl 0), vt.2 (.inl 0))),
  (delayed.solve 2 (1 / 2)).map (fun vt => (vt.1 (.inl 0), vt.2 (.inl 0))))

/-- A chain whose entire supplied state domain is already terminal. -/
def terminalOnly : Chain 0 1 where
  transient := Fin.elim0
  terminal := Fin.elim0
  normalized := by intro i; exact Fin.elim0 i
  reward := fun _ => 7

/-- The empty state domain is valid and imposes no hidden nonemptiness hypothesis. -/
def empty : Chain 0 0 where
  transient := Fin.elim0
  terminal := Fin.elim0
  normalized := by intro i; exact Fin.elim0 i
  reward := Fin.elim0

/-- With a nonempty transient domain and no terminals, this normalized loop is closed. -/
def noTerminals : Chain 1 0 where
  transient := fun _ _ => 1
  terminal := fun _ => Fin.elim0
  normalized := by intro i; simp
  reward := Fin.elim0

/-- Three deterministic transitions before the terminal reward is paid. -/
def deterministic : Chain 3 1 where
  transient := !![0, 1, 0; 0, 0, 1; 0, 0, 0]
  terminal := fun i _ => if i = 2 then 1 else 0
  normalized := by intro i; fin_cases i <;> norm_num [Fin.sum_univ_succ, Fin.ext_iff]
  reward := fun _ => 7

/-- A strictly positive rational exit probability that must not be rounded to zero. -/
def tinyExit : Chain 1 1 where
  transient := fun _ _ => 999999 / 1000000
  terminal := fun _ _ => 1 / 1000000
  normalized := by intro i; norm_num
  reward := fun _ => 7

/-- All successful regression certificates are computed without external bounds. -/
theorem automatic_checks : geometric.autoCheck = true ∧ twoRewards.autoCheck = true ∧
    delayed.autoCheck = true ∧ terminalOnly.autoCheck = true ∧ empty.autoCheck = true ∧
    deterministic.autoCheck = true ∧ tinyExit.autoCheck = true := by
  decide +kernel

/-- The automatic geometric solution retains reward seven and mean duration two. -/
theorem geometric_autoResult :
    geometric.autoSolve.map (fun vt => (vt.1 (.inl 0), vt.2 (.inl 0))) = .ok (7, 2) := by
  decide +kernel

/-- The automatically solved mixture preserves the original signed terminal payoffs. -/
theorem twoRewards_autoResult :
    twoRewards.autoSolve.map (fun vt => (vt.1 (.inl 0), vt.2 (.inl 0))) = .ok (1, 2) := by
  decide +kernel

/-- Automatic multi-step absorption handles a state with no immediate terminal edge. -/
theorem delayed_autoResult :
    delayed.autoSolve.map (fun vt => (vt.1 (.inl 0), vt.2 (.inl 0))) = .ok (7, 3) := by
  decide +kernel

/-- An already terminal start returns its own reward and duration zero. -/
theorem alreadyTerminal_autoResult :
    twoRewards.autoSolve.map (fun vt => (vt.1 (.inr 1), vt.2 (.inr 1))) = .ok (5, 0) := by
  decide +kernel

/-- Empty transient rows give exactly zero bound, for both empty-domain cases. -/
theorem empty_bounds : terminalOnly.autoBound = 0 ∧ empty.autoBound = 0 := by
  decide +kernel

/-- No transient states: the solver still returns the original terminal payoff. -/
theorem terminalOnly_autoResult :
    terminalOnly.autoSolve.map (fun vt => (vt.1 (.inr 0), vt.2 (.inr 0))) = .ok (7, 0) := by
  decide +kernel

/-- With no states at all the actual solver succeeds and returns empty functions. -/
theorem empty_autoResult : empty.autoSolve.map (fun _ => ()) = .ok () := by
  decide +kernel

/-- Deterministic first-hit time counts all three transitions. -/
theorem deterministic_autoResult :
    deterministic.autoSolve.map (fun vt => (vt.1 (.inl 0), vt.2 (.inl 0))) = .ok (7, 3) := by
  decide +kernel

/-- Exact arithmetic detects the tiny exit and computes its large finite mean time. -/
theorem tinyExit_autoResult :
    tinyExit.autoSolve.map (fun vt => (vt.1 (.inl 0), vt.2 (.inl 0))) = .ok (7, 1000000) := by
  decide +kernel

/-- Full-domain rejection includes the separate closed component and the chain
with no terminals. It makes no assertion that every initial state is bad. -/
theorem automatic_rejections : closedClass.autoCheck = false ∧ noTerminals.autoCheck = false := by
  decide +kernel

/-- A closed component remains an explicit error even when unreachable from state zero. -/
theorem closedClass_autoRejected : closedClass.autoSolve = .error .absorptionBound :=
  closedClass.autoSolve_of_rejected automatic_rejections.1

/-- The normalized loop with no terminal states cannot produce a successful solution. -/
theorem noTerminals_autoRejected : noTerminals.autoSolve = .error .absorptionBound :=
  noTerminals.autoSolve_of_rejected automatic_rejections.2

/-- In the rejected chain, state zero terminates in one step. -/
theorem closedClass_good_start (horizon : ℕ) : closedClass.survival (horizon + 1) 0 = 0 := by
  rw [Chain.survival_succ]
  simp [Chain.Q, closedClass, Fin.sum_univ_two]

/-- Its other state retains all transient mass forever, a concrete failure witness. -/
theorem closedClass_bad_start (horizon : ℕ) : closedClass.survival horizon 1 = 1 := by
  induction horizon with
  | zero => exact closedClass.survival_zero 1
  | succ horizon ih =>
    rw [Chain.survival_succ]
    simpa [Chain.Q, closedClass, Fin.sum_univ_two] using ih

#eval [geometric.autoSolve.map (fun vt => (vt.1 (.inl 0), vt.2 (.inl 0))),
  twoRewards.autoSolve.map (fun vt => (vt.1 (.inl 0), vt.2 (.inl 0))),
  delayed.autoSolve.map (fun vt => (vt.1 (.inl 0), vt.2 (.inl 0))),
  twoRewards.autoSolve.map (fun vt => (vt.1 (.inr 1), vt.2 (.inr 1))),
  terminalOnly.autoSolve.map (fun vt => (vt.1 (.inr 0), vt.2 (.inr 0))),
  deterministic.autoSolve.map (fun vt => (vt.1 (.inl 0), vt.2 (.inl 0))),
  tinyExit.autoSolve.map (fun vt => (vt.1 (.inl 0), vt.2 (.inl 0))),
  closedClass.autoSolve.map (fun vt => (vt.1 (.inl 0), vt.2 (.inl 0))),
  noTerminals.autoSolve.map (fun vt => (vt.1 (.inl 0), vt.2 (.inl 0)))]

#eval ((terminalOnly.autoBound : ℚ), (empty.autoBound : ℚ), empty.autoSolve.map (fun _ => ()))

end Regression
end Examples.ExtensiveGame.FiniteMarkovChain
