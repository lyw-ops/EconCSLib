...
the compact state space. `finiteAction` and `finiteRepresentedInfo` are
typeclass certificates stored propositionally and can be installed locally by
an algorithm. Player finiteness, perfect recall, chance laws, preference
orders, and decidable comparisons are intentionally separate. -/
