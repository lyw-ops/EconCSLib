#!/usr/bin/env python3
"""Zero-model terminal Lean-check contract for Stage 5B Sol REP-003."""
...
def load_events(workspace: Path) -> list[dict[str, Any]]:
    ...
    if not path.is_file():
        raise TerminalCheckError("required Sol REP-003 Lean-check evidence is missing")
    lines = [line for line in path.read_text(encoding="utf-8").splitlines() if line.strip()]
    if not lines:
        raise TerminalCheckError("required Sol REP-003 Lean-check evidence is empty")
    events: list[dict[str, Any]] = []
    for line in lines:
        try:
            value = json.loads(line)
        except json.JSONDecodeError as exc:
            raise TerminalCheckError("Sol REP-003 Lean-check evidence is malformed") from exc
        if not isinstance(value, dict):
            raise TerminalCheckError("Sol REP-003 Lean-check event is not an object")
        events.append(value)
    return events
    ...
    *,
    ...
    for sequence, event in enumerate(events, start=1):
        if event.get("schema_version") != "2.0.0":
            raise TerminalCheckError("Sol REP-003 Lean-check schema is invalid")
        if event.get("sequence") != sequence:
            raise TerminalCheckError("Sol REP-003 Lean-check sequence is not contiguous")
        if event.get("previous_event_sha256") != previous:
            raise TerminalCheckError("Sol REP-003 Lean-check hash chain is invalid")
        if event.get("checker_sha256") != checker_sha256:
            raise TerminalCheckError("Sol REP-003 Lean check used a different checker")
        if event.get("event_sha256") != event_sha256(event):
            raise TerminalCheckError("Sol REP-003 Lean-check event hash is invalid")
        for key in (
            "candidate_sha256",
            "guidance_sha256",
            "stdout_sha256",
            ...
            if not _is_hash(event.get(key)):
                raise TerminalCheckError(f"Sol REP-003 Lean-check {key} is invalid")
        if event.get("python_invocation") != EXPECTED_PYTHON_INVOCATION:
            raise TerminalCheckError("Sol REP-003 checker invocation drifted")
        if event.get("python_executable") != EXPECTED_PYTHON_EXECUTABLE:
            raise TerminalCheckError("Sol REP-003 checker executable drifted")
        if event.get("python_version") != EXPECTED_PYTHON_VERSION:
            raise TerminalCheckError("Sol REP-003 checker Python version drifted")
        if event.get("command") != EXPECTED_CHECK_COMMAND:
            raise TerminalCheckError("Sol REP-003 Lean command drifted")
        exit_code = event.get("exit_code")
        failed = event.get("failed")
        if isinstance(exit_code, bool) or not isinstance(exit_code, int):
            raise TerminalCheckError("Sol REP-003 Lean-check exit status is invalid")
        if not isinstance(failed, bool) or failed != (exit_code != 0):
            raise TerminalCheckError("Sol REP-003 Lean-check failure flag is inconsistent")
        ...
        try:
            ...
            raise TerminalCheckError("Sol REP-003 Lean-check timestamp is malformed") from exc
        if instant.utcoffset() != dt.timedelta(0):
            raise TerminalCheckError("Sol REP-003 Lean-check timestamp is not UTC")
        previous = str(event["event_sha256"])
    if events[-1].get("candidate_sha256") != final_candidate_sha256:
        raise TerminalCheckError(
            "Sol REP-003 final checker event does not match final candidate"
        )
...
def run_terminal_checker(
    ...
    *,
    ...
    if not candidate.is_file():
        raise TerminalCheckError("Sol REP-003 final candidate is missing")
    if not checker.is_file():
        raise TerminalCheckError("Sol REP-003 immutable Lean checker is missing")
    checker_hash = file_sha256(checker)
    if checker_hash != expected_checker_sha256:
        raise TerminalCheckError("Sol REP-003 immutable Lean checker changed during rollout")
    ...
    if candidate_after != candidate_before:
        raise TerminalCheckError("Sol REP-003 checker changed the final candidate")

    events = load_events(workspace)
    if len(events) != prior_count + 1:
        raise TerminalCheckError(
            "Sol REP-003 terminal checker did not append exactly one evidence event"
        )
    validate_events(
        events,
    ...
    if final_event.get("exit_code") != completed.returncode:
        raise TerminalCheckError("Sol REP-003 terminal checker exit status drifted")
    for key, observed in (
        ...
        if final_event.get(key) != observed:
            raise TerminalCheckError(f"Sol REP-003 terminal checker {key} drifted")
    return {
