#!/usr/bin/env python3
"""Fail-closed read-only audit for one Stage 5B Sol REP-003 run."""
...
RUNS_ROOT = (SIDECAR_ROOT / ".runtime" / "stage5b-sol-rep003-runs").resolve()
ATTEMPT_LEDGER_PATH = (
...
EXPECTED_UPSTREAM_RUNNER_SHA256 = "06bc35d16f0ab80c48b27491c83b3830c4e2449c5ccedb83fe2f025ca3f9911d"
EXPECTED_UPSTREAM_HOOK_WRITER_SHA256 = "f50efdfbf5f2f18d9c805bd393eda8086e696bc351bf1fbf80060c2477d80fab"
_HEX64 = re.compile(r"[0-9a-f]{64}")


def _load_legacy():
    path = SIDECAR_ROOT / "scripts" / "audit_stage5a.py"
    spec = importlib.util.spec_from_file_location("eve_stage5b_sol_rep003_audit_core", path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Could not load frozen DEV-002 auditor {path}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["eve_stage5b_sol_rep003_audit_core"] = module
...
def _validate_hook_guard(launch: dict[str, Any]) -> None:
    """Reject a launch whose real external-hook guard is absent or unauditable."""
    guard = launch.get("pre_reservation_hook_guard")
    if not isinstance(guard, dict) or guard.get("schema_version") != "1.0.0":
        raise AuditError("Sol REP-003 pre-reservation hook guard is missing or malformed")
    expected = {
        "phase": "pre-reservation-pre-formal-root-pre-model",
        "operation": "write_repo_codex_hooks/write_codex_hooks_file/Path.write_text",
        "upstream_runner_sha256": EXPECTED_UPSTREAM_RUNNER_SHA256,
        "upstream_hook_writer_sha256": EXPECTED_UPSTREAM_HOOK_WRITER_SHA256,
    ...
    for key, value in expected.items():
        if guard.get(key) != value:
            raise AuditError(f"Sol REP-003 hook guard {key} mismatch")
    checkout = guard.get("eve_checkout")
    hooks_path = guard.get("hooks_path")
    if (
        not isinstance(checkout, str)
        or not isinstance(hooks_path, str)
        or Path(hooks_path).resolve() != Path(checkout).resolve() / ".codex" / "hooks.json"
    ):
        raise AuditError("Sol REP-003 hook guard did not target the external checkout")
    try:
        Path(checkout).resolve().relative_to(SIDECAR_ROOT.parents[1].resolve())
    except ValueError:
        pass
    else:
        raise AuditError("Sol REP-003 hook guard checkout is not external")
    before = guard.get("content_before_sha256")
    if before is not None and not _is_hash(before):
        raise AuditError("Sol REP-003 hook guard prior content hash is invalid")
    if not _is_hash(guard.get("content_after_sha256")):
        raise AuditError("Sol REP-003 hook guard final content hash is invalid")
    for prefix in ("formal_state", "historical_state"):
        before_hash = guard.get(f"{prefix}_before_sha256")
        after_hash = guard.get(f"{prefix}_after_sha256")
        if not _is_hash(before_hash) or after_hash != before_hash:
            raise AuditError(f"Sol REP-003 hook guard {prefix} projection changed")
    quiescence = guard.get("quiescence")
    if not isinstance(quiescence, dict) or set(quiescence) != {
        "formal_before", "formal_after", "historical_before", "historical_after"
    }:
        raise AuditError("Sol REP-003 hook guard quiescence evidence is malformed")
    for barrier in quiescence.values():
        if (
            not isinstance(barrier, dict)
            or barrier.get("stable_samples_required") != 3
            or barrier.get("timeout_seconds") != 30.0
            ...
            raise AuditError("Sol REP-003 hook guard quiescence evidence is invalid")
...
def _validate_launch_preflight(launch: dict[str, Any]) -> None:
    ...
    _validate_hook_guard(launch)
    if launch.get("clean_lean_source_commit") != SOURCE_COMMIT:
        raise AuditError("Sol REP-003 launch clean Lean source commit drifted")
    preflight = launch.get("safe_preflight")
    if not isinstance(preflight, dict) or preflight.get("schema_version") != "2.0.0":
        raise AuditError("Sol REP-003 launch safe preflight is missing or malformed")
    if preflight.get("route") != launch.get("route"):
        raise AuditError("Sol REP-003 safe preflight route mismatch")
    expected = {
    ...
    for key, value in expected.items():
        if preflight.get(key) != value:
            raise AuditError(f"Sol REP-003 safe preflight {key} mismatch")
    for key in (
        "candidate_sha256",
        "lean_stdout_sha256",
        "lean_stderr_sha256",
        ...
        if not _is_hash(preflight.get(key)):
            raise AuditError(f"Sol REP-003 safe preflight {key} is invalid")
    if preflight["formal_state_before_sha256"] != preflight["formal_state_after_sha256"]:
        raise AuditError("Sol REP-003 formal-state projection changed across preflight")
    if preflight["historical_state_before_sha256"] != preflight["historical_state_after_sha256"]:
        raise AuditError("Sol REP-003 historical-state projection changed across preflight")
    quiescence = preflight.get("quiescence")
    if not isinstance(quiescence, dict) or set(quiescence) != {
        "formal_before", "formal_after", "historical_before", "historical_after"
    }:
        raise AuditError("Sol REP-003 quiescence evidence is missing or malformed")
    for barrier in quiescence.values():
        if (
            not isinstance(barrier, dict)
            or barrier.get("stable_samples_required") != 3
            ...
            raise AuditError("Sol REP-003 quiescence barrier evidence is invalid")
    exit_code = preflight.get("lean_exit_code")
    if isinstance(exit_code, bool) or not isinstance(exit_code, int) or exit_code == 0:
        raise AuditError("Sol REP-003 safe preflight did not record a real Lean failure")
    byte_counts = [
    ...
    if any(
        ...
        raise AuditError("Sol REP-003 safe preflight output evidence is invalid")
...
def _validate_terminal_events(
    ...
    if terminal_iterations != list(range(1, iterations + 1)):
        raise AuditError("Sol REP-003 terminal rollout telemetry is incomplete")
    if len({str(event.get("workspace_id")) for event in terminals}) != len(terminals):
        raise AuditError("Sol REP-003 terminal rollout workspace telemetry is duplicated")
    ...
    for terminal in terminals:
        ...
        if terminal.get("check_evidence_required") is not True:
            raise AuditError("Sol REP-003 terminal event did not require checker evidence")
        if status == "CHECK_EVIDENCE_VALIDATED":
            if terminal.get("check_evidence_validated") is not True:
                raise AuditError("Sol REP-003 successful terminal event lacks validated evidence")
            if terminal.get("terminal_checker_phase") != "post-agent-pre-evaluation":
                raise AuditError("Sol REP-003 terminal checker phase drifted")
            ...
            if not _is_hash(checker_hash) or not _is_hash(candidate_hash):
                raise AuditError("Sol REP-003 terminal checker or candidate hash is invalid")
            if not isinstance(checks, list):
                raise AuditError("Sol REP-003 terminal Lean-check evidence is not a list")
            ...
            if terminal.get("terminal_check_event_sha256") != checks[-1].get(
                ...
                raise AuditError("Sol REP-003 terminal checker event join drifted")
            completed = completed_by_iteration.get(iteration)
            if completed is None:
                raise AuditError("Sol REP-003 successful terminal event has no completed rollout")
            for key in (
                "workspace_id",
                ...
                if completed.get(key) != terminal.get(key):
                    raise AuditError(f"Sol REP-003 terminal/completed {key} mismatch")
            ...
            if status == "RUN_FAILED_CHECK_EVIDENCE_CONTRACT" and terminal.get(
                ...
                raise AuditError("Sol REP-003 rejected checker evidence was marked valid")
            if not isinstance(terminal.get("failure_stage"), str) or not isinstance(
                terminal.get("failure_message"), str
            ):
                raise AuditError("Sol REP-003 rejected terminal event lacks failure context")
            if any(
                event.get("event") == "optimizer_candidate_produced"
                and int(event.get("source_iteration", 0)) == iteration
                for event in events
            ):
                raise AuditError("Sol REP-003 rejected rollout produced an optimizer")
            failures.append(terminal)
        else:
            raise AuditError("Sol REP-003 terminal rollout status is invalid")
    return failures
...
def _prevalidate_runtime_evidence(
    ...
    if not launch_path.is_file():
        raise AuditError("Sol REP-003 launch evidence is missing")
    try:
        launch = json.loads(launch_path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise AuditError("Sol REP-003 launch evidence is malformed") from exc
    if not isinstance(launch, dict):
        raise AuditError("Sol REP-003 launch evidence is not an object")
    _validate_launch_preflight(launch)
    telemetry_path = run_root.resolve() / "stage5a-guidance-lineage.jsonl"
    if not telemetry_path.is_file():
        raise AuditError("Sol REP-003 telemetry evidence is missing")
    try:
        events = CORE.load_events(telemetry_path)
    except json.JSONDecodeError as exc:
        raise AuditError("Sol REP-003 telemetry evidence is malformed") from exc
    ...
    if len(set(workspace_ids)) != len(workspace_ids):
        raise AuditError("Sol REP-003 rollout workspace telemetry is duplicated")
    rollout_by_workspace: dict[str, dict[str, Any]] = {}
    for rollout in rollouts:
        if rollout.get("check_evidence_required") is not True:
            raise AuditError("Sol REP-003 rollout did not require Lean-check evidence")
        if rollout.get("check_evidence_validated") is not True:
            raise AuditError("Sol REP-003 rollout lacks validated Lean-check evidence")
        ...
        if not _is_hash(checker_hash) or not _is_hash(candidate_hash):
            raise AuditError("Sol REP-003 rollout checker or candidate hash is invalid")
        if not isinstance(checks, list):
            raise AuditError("Sol REP-003 local Lean-check evidence is not a list")
        validate_lean_check_chain(
            checks,
            checker_sha256=str(checker_hash),
            final_candidate_sha256=str(candidate_hash),
        ...
        if rollout.get("failure_before_guidance_change_sequences") != expected_sequences:
            raise AuditError("Sol REP-003 failure sequence evidence drifted")
        if rollout.get("failure_event_hashes") != expected_hashes:
            raise AuditError("Sol REP-003 failure-event hashes drifted")
    ...
    for produced in (
        ...
        if rollout is None:
            raise AuditError("Sol REP-003 produced optimizer has no source rollout")
        if produced.get("check_evidence_validated") is not True:
            raise AuditError("Sol REP-003 produced optimizer lacks validated checks")
        for key in (
            ...
            if produced.get(key) != rollout.get(key):
                raise AuditError(f"Sol REP-003 produced optimizer {key} mismatch")
...
def _audit_rejected_run(
    ...
    *,
    ...
    if resolved.parent != RUNS_ROOT or not CORE._is_relative_to(resolved, RUNS_ROOT):
        raise AuditError("run root is not one fresh Sol REP-003 run child")
    ...
    if protocol.get("id") != PROTOCOL_ID or protocol.get("status") != "FROZEN_NOT_YET_EXECUTED":
        raise AuditError("frozen Sol REP-003 protocol identity or status mismatch")
    expected_launch = {
    ...
    for key, value in expected_launch.items():
        if launch.get(key) != value:
            raise AuditError(f"Sol REP-003 rejected launch {key} mismatch")
    ...
    if condition not in {"static", "fixed", "evolved"}:
        raise AuditError("Sol REP-003 rejected launch condition drifted")
    if route not in {"direct", "transport"} or experiment != f"entry-game-{route}":
        raise AuditError("Sol REP-003 rejected launch task/route mismatch")
    matrix_matches = [
        cell
    ...
    if len(matrix_matches) != 1 or launch.get("matrix_ordinal") != matrix_matches[0].get(
        ...
        raise AuditError("Sol REP-003 rejected launch matrix ordinal mismatch")
    if launch.get("solver_prompt_bundle_sha256") != protocol.get(
        "solver_prompt_bundle_hashes", {}
    ).get(route):
        raise AuditError("Sol REP-003 rejected launch prompt hash mismatch")
    guidance_key = (
    ...
    if launch.get("initial_guidance_tree_sha256") != protocol.get(
        ...
        raise AuditError("Sol REP-003 rejected launch guidance hash mismatch")
    seed_path = resolved / "eve-stage5a-sampler-seed.json"
    if not seed_path.is_file():
        raise AuditError("Sol REP-003 rejected run sampler audit is missing")
    ...
    if (
        ...
        raise AuditError("Sol REP-003 rejected run sampler audit drifted")
    iterations = int(launch.get("iterations", 0))
    ...
    for event_name, message in (
        ...
        if observed != expected_iterations:
            raise AuditError(f"Sol REP-003 rejected run {message} telemetry is incomplete")
    ...
    if len(factories) != 1 or factories[0].get("provider_model_seed_controlled") is not False:
        raise AuditError("Sol REP-003 rejected run factory seed telemetry drifted")
    samples = {
        int(event.get("iteration", 0)): event
        for event in events
    ...
    for loaded in (event for event in events if event.get("event") == "solver_guidance_loaded"):
        if loaded.get("matches_sampled_optimizer") is not True or loaded.get(
            ...
            raise AuditError("Sol REP-003 rejected run loaded guidance mismatch")
        ...
        if sample is None or not any(
            ...
            raise AuditError("Sol REP-003 rejected run guidance was not sampled")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Stage 5B Sol REP-003 audit error: {exc}")
        return 2
