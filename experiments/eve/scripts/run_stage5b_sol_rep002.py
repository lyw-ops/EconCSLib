#!/usr/bin/env python3
"""Safe entrypoint for frozen Stage 5B Sol REP-002; execution needs separate authorization."""
...
RUNS_ROOT = SIDECAR_ROOT / ".runtime" / "stage5b-sol-rep002-runs"
ATTEMPT_LEDGER_PATH = (
...
SEMANTICS_PATH = SIDECAR_ROOT / "stage5b_sol_rep002_upstream_semantics.json"
LEAN_ENVIRONMENT_PATH = SIDECAR_ROOT / "stage5b_sol_rep002_lean_environment.json"
...
def _load_legacy():
    ...
    spec = importlib.util.spec_from_file_location("eve_stage5b_sol_rep002_core", path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Could not load frozen DEV-002 helper {path}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["eve_stage5b_sol_rep002_core"] = module
...
SPECS = {
    "entry-game-direct": Stage5ASpec(
        ...
        route="direct",
        config_name="entry_game_direct_stage5b_sol_rep002",
        overlay_key="stage5b_sol_rep002_entry_game_direct",
    ...
    "entry-game-transport": Stage5ASpec(
        ...
        route="transport",
        config_name="entry_game_transport_stage5b_sol_rep002",
        overlay_key="stage5b_sol_rep002_entry_game_transport",
...
def load_lean_environment(protocol: dict[str, Any]) -> dict[str, Any]:
    ...
    expected = {
        "path": "experiments/eve/stage5b_sol_rep002_lean_environment.json",
        "sha256": sha256(LEAN_ENVIRONMENT_PATH),
    }
    if record != expected:
        raise CheckError("Sol REP-002 Lean environment manifest hash drifted")
    try:
        value = json.loads(LEAN_ENVIRONMENT_PATH.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise CheckError("Sol REP-002 Lean environment manifest is invalid") from exc
    if not isinstance(value, dict):
        raise CheckError("Sol REP-002 Lean environment manifest is not an object")
    return value
...
def verify_manifest_against_source_commit(environment: dict[str, Any]) -> None:
    if environment.get("source_repository") != SOURCE_REPOSITORY:
        raise CheckError("Sol REP-002 Lean source repository drifted")
    if environment.get("source_commit") != SOURCE_COMMIT:
        raise CheckError("Sol REP-002 Lean source commit drifted")
    if environment.get("entry_modules") != [LEAN_ENTRY_MODULE]:
        raise CheckError("Sol REP-002 Lean entry modules drifted")
    actual_closure = committed_lean_import_closure((LEAN_ENTRY_MODULE,))
    if environment.get("local_import_closure") != actual_closure:
        raise CheckError("Sol REP-002 committed Lean import closure drifted")
    metadata = {
    ...
    if environment.get("build_metadata") != metadata:
        raise CheckError("Sol REP-002 committed Lake metadata drifted")
    ...
    if environment.get("dependency_checkouts") != packages:
        raise CheckError("Sol REP-002 dependency revisions drifted")
...
def verify_clean_lean_checkout(lean_root: Path, environment: dict[str, Any]) -> Path:
    ...
    if not (root / ".git").exists() or not (root / "lakefile.toml").is_file():
        raise CheckError("Sol REP-002 clean Lean checkout is unavailable")
    head = str(_run_git(root, "rev-parse", "HEAD")).strip()
    if head != SOURCE_COMMIT:
        raise CheckError("Sol REP-002 clean Lean checkout is at the wrong commit")
    status = str(_run_git(root, "status", "--porcelain", "--untracked-files=all"))
    if status.strip():
        raise CheckError("Sol REP-002 Lean checkout is not tracked-clean")
    ...
    if _normalize_remote(SOURCE_REPOSITORY) not in {
        ...
        raise CheckError("Sol REP-002 Lean checkout lacks the frozen repository remote")
    for relative, expected in environment["local_import_closure"].items():
        path = root / relative
        if not path.is_file() or sha256(path) != expected:
            raise CheckError(f"Sol REP-002 clean Lean source drifted: {relative}")
    for relative, expected in environment["build_metadata"].items():
        path = root / relative
        if not path.is_file() or sha256(path) != expected:
            raise CheckError(f"Sol REP-002 clean build metadata drifted: {relative}")
    ...
    for name, revision in sorted(environment["dependency_checkouts"].items()):
        ...
        if not checkout.is_dir():
            raise CheckError(f"Sol REP-002 dependency checkout is missing: {name}")
        if str(_run_git(checkout, "rev-parse", "HEAD")).strip() != revision:
            raise CheckError(f"Sol REP-002 dependency commit drifted: {name}")
        dirty = str(_run_git(checkout, "status", "--porcelain", "--untracked-files=no"))
        if dirty.strip():
            raise CheckError(f"Sol REP-002 dependency checkout is dirty: {name}")
    return root


def _protocol_hash() -> str:
    record = PROTOCOL_HASH_PATH.read_text(encoding="utf-8").split()
    expected_path = "experiments/eve/stage5b_sol_rep002_protocol.json"
    if len(record) != 2 or record[1] != expected_path:
        raise CheckError("Sol REP-002 detached protocol hash record is invalid")
    actual = sha256(PROTOCOL_PATH)
    if record[0] != actual:
        raise CheckError("Sol REP-002 protocol hash mismatch")
    return actual
...
def verify_protocol_assets(lean_checkout: Path | None = None) -> dict[str, Any]:
    ...
    if protocol.get("edit_surfaces") != {
        ...
        raise CheckError("Sol REP-002 edit surfaces are not exact")
    ...
    if not all(
        ...
        raise CheckError("Sol REP-002 checker evidence contract is not fail closed")
...
def _required_paths(spec: Stage5ASpec) -> tuple[Path, ...]:
    ...
    paths = (
        ...
        CONFIG_ROOT / "optimizer" / f"entry_game_{spec.route}_stage5b_sol_rep002.yaml",
        *(CONFIG_ROOT / "stage5a_condition" / f"{name}.yaml" for name in CONDITIONS),
        ...
        SIDECAR_ROOT / "scripts" / "run_stage5b_sol_rep002_eve.py",
        SIDECAR_ROOT / "scripts" / "stage5b_sol_rep002_terminal.py",
        SIDECAR_ROOT / "scripts" / "audit_stage5b_sol_rep002.py",
        SIDECAR_ROOT / "scripts" / "verify_stage5b_sol_rep002_protocol.py",
        PROTOCOL_PATH,
        PROTOCOL_HASH_PATH,
        SEMANTICS_PATH,
...
def verify_sidecar_assets(spec: Stage5ASpec, lean_root: Path) -> None:
    ...
    if missing:
        raise CheckError("Missing Sol REP-002 assets: " + ", ".join(missing))
    ...
    if case.get("editable_files") != ["Candidate.lean"]:
        raise CheckError("Sol REP-002 solver file boundary drifted")
    if case.get("editable_folders") != []:
        raise CheckError("Sol REP-002 solver folder boundary drifted")


def validate_selection(*, protocol_id: str, condition: str, experiment_seed: int) -> None:
    if protocol_id != PROTOCOL_ID:
        raise CheckError("Sol REP-002 protocol id mismatch")
    if condition not in CONDITIONS:
        raise CheckError("Sol REP-002 condition is not frozen")
    if experiment_seed not in SEEDS:
        raise CheckError("Sol REP-002 experiment seed is not frozen")
...
def _dependency_probe(identity, spec: Stage5ASpec, condition: str, seed: int, lean_root: Path) -> bool:
    ...
    if uv is None or not (identity.root / ".venv" / "bin" / "python").is_file():
        return False
    with tempfile.TemporaryDirectory(prefix="econcslib-stage5b-sol-rep002-probe-") as raw:
        temp_root = Path(raw)
        guidance = (
...
wrapper_spec = importlib.util.spec_from_file_location('econcslib_stage5b_sol_rep002_wrapper', {str(SIDECAR_ROOT / 'scripts' / 'run_stage5b_sol_rep002_eve.py')!r})
...
def _build_frozen_lean_entry(protocol: dict[str, Any], lean_root: Path) -> None:
    ...
    if target != LEAN_ENTRY_MODULE:
        raise CheckError("Sol REP-002 prelaunch Lean target drifted")
    completed = subprocess.run(
        ["lake", "build", target],
        cwd=lean_root,
        stdout=subprocess.PIPE,
    ...
    if completed.returncode != 0:
        ...
        raise CheckError(f"Sol REP-002 clean Lean entry did not build:\n{detail}")
...
def safe_preflight(spec: Stage5ASpec, lean_root: Path, toolchain: Path) -> dict[str, Any]:
    ...
    formal_before, formal_before_barrier = _wait_for_quiescent_snapshot(
        ...
        label="Sol REP-002 formal state before preflight",
    )
    ...
    with tempfile.TemporaryDirectory(prefix="econcslib-stage5b-sol-rep002-preflight-") as raw:
        workspace = Path(raw).resolve()
        temp_path = workspace
        if _is_relative_to(workspace, SIDECAR_ROOT.resolve()):
            raise CheckError("Sol REP-002 preflight workspace is not disposable")
        ...
        if completed.returncode == 0:
            raise CheckError("Sol REP-002 preflight did not exercise a real Lean failure")
        events_path = workspace / ".scaling_evolve" / "stage5a-lean-checks.jsonl"
        if not events_path.is_file():
            raise CheckError("Sol REP-002 preflight checker recorded no event")
        lines = [line for line in events_path.read_text(encoding="utf-8").splitlines() if line]
        if len(lines) != 1:
            raise CheckError("Sol REP-002 preflight checker event count is not one")
        try:
            event = json.loads(lines[0])
        except json.JSONDecodeError as exc:
            raise CheckError("Sol REP-002 preflight checker event is malformed") from exc
        expected = {
        ...
        for key, value in expected.items():
            if event.get(key) != value:
                raise CheckError(f"Sol REP-002 preflight event mismatch: {key}")
        payload = {key: value for key, value in event.items() if key != "event_sha256"}
        digest = hashlib.sha256(
            json.dumps(payload, sort_keys=True, separators=(",", ":"), ensure_ascii=False).encode("utf-8")
        ).hexdigest()
        if event.get("event_sha256") != digest:
            raise CheckError("Sol REP-002 preflight event hash is invalid")
        report = {
            "schema_version": "2.0.0",
            "route": spec.route,
    ...
    if temp_path is None or temp_path.exists():
        raise CheckError("Sol REP-002 disposable preflight workspace was not removed")
    formal_after, formal_after_barrier = _wait_for_quiescent_snapshot(
        lambda: _execution_state_snapshot(RUNS_ROOT, ATTEMPT_LEDGER_PATH),
        label="Sol REP-002 formal state after preflight",
    )
    if formal_before != formal_after:
        raise CheckError("Sol REP-002 preflight changed projected formal execution state")
    ...
    if historical_before != historical_after:
        raise CheckError("Sol REP-002 preflight changed projected historical state")
...
def _new_run_root() -> Path:
    ...
    if candidate.parent != RUNS_ROOT.resolve() or candidate.exists():
        raise CheckError("Sol REP-002 fresh run root is invalid or already exists")
    return candidate
...
def execute(
    ...
    *,
    ...
    if not acknowledge_model_quota:
        raise CheckError("Sol REP-002 execution requires --acknowledge-model-quota")
    if not _dependency_probe(identity, spec, condition, seed, lean_root):
        raise CheckError("Sol REP-002 dependencies or Hydra composition are unavailable")
    ...
    if tree_sha256(guidance_root) != protocol["initial_guidance_tree_hashes"][guidance_key]:
        raise CheckError("Sol REP-002 initial guidance tree hash mismatch")
    ...
    command = [
        ...
        "--no-sync",
        "python",
        str(SIDECAR_ROOT / "scripts" / "run_stage5b_sol_rep002_eve.py"),
    ...
    if completed.returncode == 0:
        audit = subprocess.run(
            [
                sys.executable,
                str(SIDECAR_ROOT / "scripts" / "audit_stage5b_sol_rep002.py"),
                "--run-root",
                str(run_root),
                "--output",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        raise CheckError("No Sol REP-002 action selected")
    except (CheckError, OSError, subprocess.SubprocessError, json.JSONDecodeError) as exc:
        print(f"Stage 5B Sol REP-002 sidecar error: {exc}", file=sys.stderr)
        return 2
