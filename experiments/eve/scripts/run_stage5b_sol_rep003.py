#!/usr/bin/env python3
"""Safe entrypoint for frozen Stage 5B Sol REP-003; execution needs separate authorization."""
...
RUNS_ROOT = SIDECAR_ROOT / ".runtime" / "stage5b-sol-rep003-runs"
ATTEMPT_LEDGER_PATH = (
    SIDECAR_ROOT / ".runtime" / "stage5b-sol-rep003-attempt-ledger.sqlite3"
)
PROTOCOL_PATH = SIDECAR_ROOT / "stage5b_sol_rep003_protocol.json"
PROTOCOL_HASH_PATH = SIDECAR_ROOT / "stage5b_sol_rep003_protocol.sha256"
SEMANTICS_PATH = SIDECAR_ROOT / "stage5b_sol_rep003_upstream_semantics.json"
LEAN_ENVIRONMENT_PATH = SIDECAR_ROOT / "stage5b_sol_rep003_lean_environment.json"
PROTOCOL_ID = "EVE-STAGE5B-SOL-ENTRY-GAME-GUIDANCE-LIVENESS-REP-003"
...
UPSTREAM_RUNNER_RELATIVE = Path("src/scaling_evolve/algorithms/eve/runner.py")
UPSTREAM_RUNNER_SHA256 = "06bc35d16f0ab80c48b27491c83b3830c4e2449c5ccedb83fe2f025ca3f9911d"
UPSTREAM_HOOK_WRITER_RELATIVE = Path(
    "src/scaling_evolve/providers/agent/codex_hooks.py"
)
UPSTREAM_HOOK_WRITER_SHA256 = "f50efdfbf5f2f18d9c805bd393eda8086e696bc351bf1fbf80060c2477d80fab"
...
def _load_legacy():
    ...
    spec = importlib.util.spec_from_file_location("eve_stage5b_sol_rep003_core", path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Could not load frozen DEV-002 helper {path}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["eve_stage5b_sol_rep003_core"] = module
...
SPECS = {
    "entry-game-direct": Stage5ASpec(
        ...
        route="direct",
        config_name="entry_game_direct_stage5b_sol_rep003",
        overlay_key="stage5b_sol_rep003_entry_game_direct",
    ...
    "entry-game-transport": Stage5ASpec(
        ...
        route="transport",
        config_name="entry_game_transport_stage5b_sol_rep003",
        overlay_key="stage5b_sol_rep003_entry_game_transport",
...
def load_lean_environment(protocol: dict[str, Any]) -> dict[str, Any]:
    ...
    expected = {
        "path": "experiments/eve/stage5b_sol_rep003_lean_environment.json",
        "sha256": sha256(LEAN_ENVIRONMENT_PATH),
    }
    if record != expected:
        raise CheckError("Sol REP-003 Lean environment manifest hash drifted")
    try:
        value = json.loads(LEAN_ENVIRONMENT_PATH.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise CheckError("Sol REP-003 Lean environment manifest is invalid") from exc
    if not isinstance(value, dict):
        raise CheckError("Sol REP-003 Lean environment manifest is not an object")
    return value
...
def verify_manifest_against_source_commit(environment: dict[str, Any]) -> None:
    if environment.get("source_repository") != SOURCE_REPOSITORY:
        raise CheckError("Sol REP-003 Lean source repository drifted")
    if environment.get("source_commit") != SOURCE_COMMIT:
        raise CheckError("Sol REP-003 Lean source commit drifted")
    if environment.get("entry_modules") != [LEAN_ENTRY_MODULE]:
        raise CheckError("Sol REP-003 Lean entry modules drifted")
    actual_closure = committed_lean_import_closure((LEAN_ENTRY_MODULE,))
    if environment.get("local_import_closure") != actual_closure:
        raise CheckError("Sol REP-003 committed Lean import closure drifted")
    metadata = {
    ...
    if environment.get("build_metadata") != metadata:
        raise CheckError("Sol REP-003 committed Lake metadata drifted")
    ...
    if environment.get("dependency_checkouts") != packages:
        raise CheckError("Sol REP-003 dependency revisions drifted")
...
def verify_clean_lean_checkout(lean_root: Path, environment: dict[str, Any]) -> Path:
    ...
    if not (root / ".git").exists() or not (root / "lakefile.toml").is_file():
        raise CheckError("Sol REP-003 clean Lean checkout is unavailable")
    head = str(_run_git(root, "rev-parse", "HEAD")).strip()
    if head != SOURCE_COMMIT:
        raise CheckError("Sol REP-003 clean Lean checkout is at the wrong commit")
    status = str(_run_git(root, "status", "--porcelain", "--untracked-files=all"))
    if status.strip():
        raise CheckError("Sol REP-003 Lean checkout is not tracked-clean")
    ...
    if _normalize_remote(SOURCE_REPOSITORY) not in {
        ...
        raise CheckError("Sol REP-003 Lean checkout lacks the frozen repository remote")
    for relative, expected in environment["local_import_closure"].items():
        path = root / relative
        if not path.is_file() or sha256(path) != expected:
            raise CheckError(f"Sol REP-003 clean Lean source drifted: {relative}")
    for relative, expected in environment["build_metadata"].items():
        path = root / relative
        if not path.is_file() or sha256(path) != expected:
            raise CheckError(f"Sol REP-003 clean build metadata drifted: {relative}")
    ...
    for name, revision in sorted(environment["dependency_checkouts"].items()):
        ...
        if not checkout.is_dir():
            raise CheckError(f"Sol REP-003 dependency checkout is missing: {name}")
        if str(_run_git(checkout, "rev-parse", "HEAD")).strip() != revision:
            raise CheckError(f"Sol REP-003 dependency commit drifted: {name}")
        dirty = str(_run_git(checkout, "status", "--porcelain", "--untracked-files=no"))
        if dirty.strip():
            raise CheckError(f"Sol REP-003 dependency checkout is dirty: {name}")
    return root


def _protocol_hash() -> str:
    record = PROTOCOL_HASH_PATH.read_text(encoding="utf-8").split()
    expected_path = "experiments/eve/stage5b_sol_rep003_protocol.json"
    if len(record) != 2 or record[1] != expected_path:
        raise CheckError("Sol REP-003 detached protocol hash record is invalid")
    actual = sha256(PROTOCOL_PATH)
    if record[0] != actual:
        raise CheckError("Sol REP-003 protocol hash mismatch")
    return actual
...
def verify_protocol_assets(lean_checkout: Path | None = None) -> dict[str, Any]:
    ...
    if protocol.get("edit_surfaces") != {
        ...
        raise CheckError("Sol REP-003 edit surfaces are not exact")
    hook_contract = protocol.get("pre_reservation_external_hook_guard", {})
    expected_hook_contract = {
        "phase": "pre-reservation-pre-formal-root-pre-model",
        "exact_external_checkout_path_used": True,
        "pinned_helper": "scaling_evolve.providers.agent.codex_hooks.write_repo_codex_hooks",
        "actual_operation": "write_repo_codex_hooks/write_codex_hooks_file/Path.write_text",
        "trust_helper": "scaling_evolve.providers.agent.codex_hooks.ensure_codex_hooks_trusted",
        "idempotent_double_write_required": True,
        "pinned_runner_sha256": UPSTREAM_RUNNER_SHA256,
        "pinned_hook_writer_sha256": UPSTREAM_HOOK_WRITER_SHA256,
        "runs_before_protocol_clean_source_dependency_auth_build_and_safe_preflight_gates_in_execute": True,
        "failure_model_calls": 0,
        "failure_sol_quota": 0,
        "failure_formal_run_roots": 0,
        "failure_formal_ledger_writes": 0,
        "fresh_and_historical_projected_snapshots_must_be_unchanged": True,
        "historical_generations_protected": [
            "stage5a-dev002",
            "stage5a-dev003",
            "stage5b-sol-rep001",
            "stage5b-sol-rep002",
    ...
    if hook_contract != expected_hook_contract:
        raise CheckError("Sol REP-003 pre-reservation hook contract drifted")
    ...
    if not all(
        ...
        for key in (
            ...
            "wrapper_invokes_terminal_checker_after_agent_before_evaluation",
            "post_evaluation_candidate_checker_and_event_snapshot_revalidated",
            "exactly_one_protocol_terminal_event_per_rollout",
            "rejected_rollout_emits_explicit_terminal_status",
            "rejected_rollout_cannot_produce_optimizer",
            "missing_empty_malformed_or_mismatched_fails",
            "failed_event_alone_cannot_establish_liveness",
        )
    ):
        raise CheckError("Sol REP-003 checker evidence contract is not fail closed")
...
def _required_paths(spec: Stage5ASpec) -> tuple[Path, ...]:
    ...
    paths = (
        ...
        CONFIG_ROOT / "optimizer" / f"entry_game_{spec.route}_stage5b_sol_rep003.yaml",
        *(CONFIG_ROOT / "stage5a_condition" / f"{name}.yaml" for name in CONDITIONS),
        ...
        SIDECAR_ROOT / "scripts" / "run_stage5b_sol_rep003_eve.py",
        SIDECAR_ROOT / "scripts" / "stage5b_sol_rep003_terminal.py",
        SIDECAR_ROOT / "scripts" / "audit_stage5b_sol_rep003.py",
        SIDECAR_ROOT / "scripts" / "verify_stage5b_sol_rep003_protocol.py",
        PROTOCOL_PATH,
        PROTOCOL_HASH_PATH,
        SEMANTICS_PATH,
...
def verify_sidecar_assets(spec: Stage5ASpec, lean_root: Path) -> None:
    ...
    if missing:
        raise CheckError("Missing Sol REP-003 assets: " + ", ".join(missing))
    ...
    if case.get("editable_files") != ["Candidate.lean"]:
        raise CheckError("Sol REP-003 solver file boundary drifted")
    if case.get("editable_folders") != []:
        raise CheckError("Sol REP-003 solver folder boundary drifted")


def validate_selection(*, protocol_id: str, condition: str, experiment_seed: int) -> None:
    if protocol_id != PROTOCOL_ID:
        raise CheckError("Sol REP-003 protocol id mismatch")
    if condition not in CONDITIONS:
        raise CheckError("Sol REP-003 condition is not frozen")
    if experiment_seed not in SEEDS:
        raise CheckError("Sol REP-003 experiment seed is not frozen")
...
def _dependency_probe(identity, spec: Stage5ASpec, condition: str, seed: int, lean_root: Path) -> bool:
    ...
    if uv is None or not (identity.root / ".venv" / "bin" / "python").is_file():
        return False
    with tempfile.TemporaryDirectory(prefix="econcslib-stage5b-sol-rep003-probe-") as raw:
        temp_root = Path(raw)
        guidance = (
...
wrapper_spec = importlib.util.spec_from_file_location('econcslib_stage5b_sol_rep003_wrapper', {str(SIDECAR_ROOT / 'scripts' / 'run_stage5b_sol_rep003_eve.py')!r})
...
def _build_frozen_lean_entry(protocol: dict[str, Any], lean_root: Path) -> None:
    ...
    if target != LEAN_ENTRY_MODULE:
        raise CheckError("Sol REP-003 prelaunch Lean target drifted")
    completed = subprocess.run(
        ["lake", "build", target],
        cwd=lean_root,
        stdout=subprocess.PIPE,
    ...
    if completed.returncode != 0:
        ...
        raise CheckError(f"Sol REP-003 clean Lean entry did not build:\n{detail}")
...
def _historical_state_snapshot() -> dict[str, object]:
    ...
    generations = {
        "stage5a-dev002": (
            runtime / "stage5a-dev002-runs",
            runtime / "stage5a-dev002-attempt-ledger.sqlite3",
        ),
        "stage5a-dev003": (
            runtime / "stage5a-dev003-runs",
            runtime / "stage5a-dev003-attempt-ledger.sqlite3",
        ),
        "stage5b-sol-rep001": (
            runtime / "stage5b-sol-rep001-runs",
            runtime / "stage5b-sol-rep001-attempt-ledger.sqlite3",
        ),
        "stage5b-sol-rep002": (
            runtime / "stage5b-sol-rep002-runs",
            runtime / "stage5b-sol-rep002-attempt-ledger.sqlite3",
        ),
    }
    return {
        generation: _execution_state_snapshot(runs, ledger)
        for generation, (runs, ledger) in generations.items()
    }
...
def pre_reservation_hook_guard(identity) -> dict[str, Any]:
    """Exercise pinned EvE's real hook write and trust checks before reservation."""
    root = identity.root.resolve()
    if _is_relative_to(root, REPO_ROOT.resolve()):
        raise CheckError("Sol REP-003 EvE checkout is not external to EconCSLib")
    runner_source = root / UPSTREAM_RUNNER_RELATIVE
    hook_writer_source = root / UPSTREAM_HOOK_WRITER_RELATIVE
    if not runner_source.is_file() or sha256(runner_source) != UPSTREAM_RUNNER_SHA256:
        raise CheckError("Sol REP-003 pinned EvE runner source drifted")
    if (
        not hook_writer_source.is_file()
        or sha256(hook_writer_source) != UPSTREAM_HOOK_WRITER_SHA256
    ):
        raise CheckError("Sol REP-003 pinned EvE hook writer source drifted")
    uv = shutil.which("uv")
    if uv is None or not (root / ".venv" / "bin" / "python").is_file():
        raise CheckError("Sol REP-003 pinned EvE environment is unavailable")
    ...
    formal_before, formal_before_barrier = _wait_for_quiescent_snapshot(
        ...
        label="Sol REP-003 formal state before external-hook guard",
    )
    historical_before, historical_before_barrier = _wait_for_quiescent_snapshot(
        _historical_state_snapshot,
        label="historical state before Sol REP-003 external-hook guard",
    )
    hooks_path = root / ".codex" / "hooks.json"
    before_sha256 = sha256(hooks_path) if hooks_path.is_file() else None
    script = """
import hashlib
import json
import sys
...
from scaling_evolve.providers.agent.codex_hooks import (
    ...
    is_repo_project_trusted,
    missing_hook_trust_events,
    write_repo_codex_hooks,
)

root = Path(sys.argv[1]).resolve()
first = write_repo_codex_hooks(root)
first_bytes = first.read_bytes()
second = write_repo_codex_hooks(root)
second_bytes = second.read_bytes()
ensure_codex_hooks_trusted(root)
print(json.dumps({
    "hooks_path": str(second.resolve()),
    "first_write_sha256": hashlib.sha256(first_bytes).hexdigest(),
    "second_write_sha256": hashlib.sha256(second_bytes).hexdigest(),
    "writes_byte_identical": first_bytes == second_bytes,
    "project_trusted": is_repo_project_trusted(root),
    "missing_hook_trust_events": missing_hook_trust_events(second),
}, sort_keys=True))
"""
    ...
    formal_after, formal_after_barrier = _wait_for_quiescent_snapshot(
        lambda: _execution_state_snapshot(RUNS_ROOT, ATTEMPT_LEDGER_PATH),
        label="Sol REP-003 formal state after external-hook guard",
    )
    historical_after, historical_after_barrier = _wait_for_quiescent_snapshot(
        _historical_state_snapshot,
        label="historical state after Sol REP-003 external-hook guard",
    )
    if formal_before != formal_after or historical_before != historical_after:
        raise CheckError(
            "Sol REP-003 external-hook guard changed formal or historical runtime state"
        )
    if result.returncode != 0:
        raise CheckError(
            "Sol REP-003 pre-reservation external hook write/trust guard failed closed"
        )
    lines = [line for line in result.stdout.splitlines() if line.strip()]
    try:
        evidence = json.loads(lines[-1])
    except (IndexError, json.JSONDecodeError) as exc:
        raise CheckError("Sol REP-003 external-hook guard evidence is invalid") from exc
    expected_path = str(hooks_path.resolve())
    if evidence.get("hooks_path") != expected_path:
        raise CheckError("Sol REP-003 external-hook guard used the wrong checkout path")
    if evidence.get("writes_byte_identical") is not True:
        raise CheckError("Sol REP-003 external-hook guard is not idempotent")
    first_hash = evidence.get("first_write_sha256")
    if (
        not isinstance(first_hash, str)
        or evidence.get("second_write_sha256") != first_hash
        or not hooks_path.is_file()
        or sha256(hooks_path) != first_hash
    ):
        raise CheckError("Sol REP-003 external-hook payload evidence drifted")
    if (
        evidence.get("project_trusted") is not True
        or evidence.get("missing_hook_trust_events") != []
    ):
        raise CheckError("Sol REP-003 external-hook trust conditions are not satisfied")
    return {
        "schema_version": "1.0.0",
        "phase": "pre-reservation-pre-formal-root-pre-model",
        "eve_checkout": str(root),
        "hooks_path": expected_path,
        "operation": "write_repo_codex_hooks/write_codex_hooks_file/Path.write_text",
        "upstream_runner_sha256": sha256(runner_source),
        "upstream_hook_writer_sha256": sha256(hook_writer_source),
        "content_before_sha256": before_sha256,
        "content_after_sha256": first_hash,
        ...
        "formal_state_before_sha256": _canonical_sha256(formal_before),
        "formal_state_after_sha256": _canonical_sha256(formal_after),
        "historical_state_before_sha256": _canonical_sha256(historical_before),
        "historical_state_after_sha256": _canonical_sha256(historical_after),
        "quiescence": {
...
def safe_preflight(spec: Stage5ASpec, lean_root: Path, toolchain: Path) -> dict[str, Any]:
    ...
    formal_before, formal_before_barrier = _wait_for_quiescent_snapshot(
        lambda: _execution_state_snapshot(RUNS_ROOT, ATTEMPT_LEDGER_PATH),
        label="Sol REP-003 formal state before preflight",
    )
    historical_before, historical_before_barrier = _wait_for_quiescent_snapshot(
        _historical_state_snapshot,
        label="DEV-002/DEV-003/REP-001/REP-002 historical state before preflight",
    )
    ...
    with tempfile.TemporaryDirectory(prefix="econcslib-stage5b-sol-rep003-preflight-") as raw:
        workspace = Path(raw).resolve()
        temp_path = workspace
        if _is_relative_to(workspace, SIDECAR_ROOT.resolve()):
            raise CheckError("Sol REP-003 preflight workspace is not disposable")
        ...
        if completed.returncode == 0:
            raise CheckError("Sol REP-003 preflight did not exercise a real Lean failure")
        events_path = workspace / ".scaling_evolve" / "stage5a-lean-checks.jsonl"
        if not events_path.is_file():
            raise CheckError("Sol REP-003 preflight checker recorded no event")
        lines = [line for line in events_path.read_text(encoding="utf-8").splitlines() if line]
        if len(lines) != 1:
            raise CheckError("Sol REP-003 preflight checker event count is not one")
        try:
            event = json.loads(lines[0])
        except json.JSONDecodeError as exc:
            raise CheckError("Sol REP-003 preflight checker event is malformed") from exc
        expected = {
        ...
        for key, value in expected.items():
            if event.get(key) != value:
                raise CheckError(f"Sol REP-003 preflight event mismatch: {key}")
        payload = {key: value for key, value in event.items() if key != "event_sha256"}
        digest = hashlib.sha256(
            json.dumps(payload, sort_keys=True, separators=(",", ":"), ensure_ascii=False).encode("utf-8")
        ).hexdigest()
        if event.get("event_sha256") != digest:
            raise CheckError("Sol REP-003 preflight event hash is invalid")
        report = {
            "schema_version": "2.0.0",
            "route": spec.route,
    ...
    if temp_path is None or temp_path.exists():
        raise CheckError("Sol REP-003 disposable preflight workspace was not removed")
    formal_after, formal_after_barrier = _wait_for_quiescent_snapshot(
        lambda: _execution_state_snapshot(RUNS_ROOT, ATTEMPT_LEDGER_PATH),
        label="Sol REP-003 formal state after preflight",
    )
    if formal_before != formal_after:
        raise CheckError("Sol REP-003 preflight changed projected formal execution state")
    historical_after, historical_after_barrier = _wait_for_quiescent_snapshot(
        _historical_state_snapshot,
        label="DEV-002/DEV-003/REP-001/REP-002 historical state after preflight",
    )
    if historical_before != historical_after:
        raise CheckError("Sol REP-003 preflight changed projected historical state")
...
def check(identity, spec: Stage5ASpec, *, condition: str, seed: int, lean_checkout: Path) -> int:
    ...
    hook_guard = pre_reservation_hook_guard(identity)
    ...
    tools = {
        ...
        "pre-reservation external hook write/trust guard": (
            hook_guard["idempotent_double_write_verified"]
            and hook_guard["project_trusted"]
        ),
        "Sol REP-003 Hydra and wrapper probe": _dependency_probe(
            identity, spec, condition, seed, lean_root
        ),
        "real immutable-checker preflight": preflight["recorded_event_sha256"] is not None,
    }
    print(f"Stage 5B Sol REP-003 protocol: frozen ({_protocol_hash()})")
    ...
    print(
        json.dumps(
            {"pre_reservation_hook_guard": hook_guard, "safe_preflight": preflight},
            sort_keys=True,
        )
    )
...
def dry_run(identity, spec: Stage5ASpec, *, condition: str, seed: int, lean_checkout: Path) -> int:
    ...
    preview = {
        ...
        "external_hook_guard_dispatched": False,
        "external_hook_guard_runs_before_reservation_on_execute": True,
...
def _new_run_root() -> Path:
    ...
    if candidate.parent != RUNS_ROOT.resolve() or candidate.exists():
        raise CheckError("Sol REP-003 fresh run root is invalid or already exists")
    return candidate
...
def execute(
    ...
    *,
    ...
    if not acknowledge_model_quota:
        raise CheckError("Sol REP-003 execution requires --acknowledge-model-quota")
    hook_guard = pre_reservation_hook_guard(identity)
    protocol = verify_protocol_assets(lean_checkout)
    lean_root = verify_clean_lean_checkout(
        lean_checkout, load_lean_environment(protocol)
    ...
    if not _dependency_probe(identity, spec, condition, seed, lean_root):
        raise CheckError("Sol REP-003 dependencies or Hydra composition are unavailable")
    ...
    if tree_sha256(guidance_root) != protocol["initial_guidance_tree_hashes"][guidance_key]:
        raise CheckError("Sol REP-003 initial guidance tree hash mismatch")
    ...
    command = [
        ...
        "--no-sync",
        "python",
        str(SIDECAR_ROOT / "scripts" / "run_stage5b_sol_rep003_eve.py"),
    ...
    launch = {
        ...
        "pre_reservation_hook_guard": hook_guard,
        "retry": False,
        "resume": False,
        "import": False,
        "provider_model_seed_controlled": False,
    ...
    if completed.returncode == 0:
        audit = subprocess.run(
            [
                sys.executable,
                str(SIDECAR_ROOT / "scripts" / "audit_stage5b_sol_rep003.py"),
                "--run-root",
                str(run_root),
                "--output",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        raise CheckError("No Sol REP-003 action selected")
    except (CheckError, OSError, subprocess.SubprocessError, json.JSONDecodeError) as exc:
        print(f"Stage 5B Sol REP-003 sidecar error: {exc}", file=sys.stderr)
        return 2
