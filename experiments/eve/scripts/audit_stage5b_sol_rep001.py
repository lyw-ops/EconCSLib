#!/usr/bin/env python3
"""Fail-closed read-only audit for one Stage 5B Sol REP-001 run."""
...
RUNS_ROOT = (SIDECAR_ROOT / ".runtime" / "stage5b-sol-rep001-runs").resolve()
ATTEMPT_LEDGER_PATH = (
...
def _load_legacy():
    ...
    spec = importlib.util.spec_from_file_location("eve_stage5b_sol_rep001_audit_core", path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Could not load frozen DEV-002 auditor {path}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["eve_stage5b_sol_rep001_audit_core"] = module
...
def _validate_launch_preflight(launch: dict[str, Any]) -> None:
    ...
    if launch.get("clean_lean_source_commit") != SOURCE_COMMIT:
        raise AuditError("Sol REP-001 launch clean Lean source commit drifted")
    preflight = launch.get("safe_preflight")
    if not isinstance(preflight, dict) or preflight.get("schema_version") != "2.0.0":
        raise AuditError("Sol REP-001 launch safe preflight is missing or malformed")
    if preflight.get("route") != launch.get("route"):
        raise AuditError("Sol REP-001 safe preflight route mismatch")
    expected = {
    ...
    for key, value in expected.items():
        if preflight.get(key) != value:
            raise AuditError(f"Sol REP-001 safe preflight {key} mismatch")
    for key in (
        "candidate_sha256",
        "lean_stdout_sha256",
        "lean_stderr_sha256",
        ...
        if not _is_hash(preflight.get(key)):
            raise AuditError(f"Sol REP-001 safe preflight {key} is invalid")
    if preflight["formal_state_before_sha256"] != preflight["formal_state_after_sha256"]:
        raise AuditError("Sol REP-001 formal-state projection changed across preflight")
    if preflight["historical_state_before_sha256"] != preflight["historical_state_after_sha256"]:
        raise AuditError("Sol REP-001 historical-state projection changed across preflight")
    ...
    if not isinstance(quiescence, dict) or set(quiescence) != {
        ...
        raise AuditError("Sol REP-001 quiescence evidence is missing or malformed")
    for barrier in quiescence.values():
        if (
            ...
            raise AuditError("Sol REP-001 quiescence barrier evidence is invalid")
    exit_code = preflight.get("lean_exit_code")
    if isinstance(exit_code, bool) or not isinstance(exit_code, int) or exit_code == 0:
        raise AuditError("Sol REP-001 safe preflight did not record a real Lean failure")
    byte_counts = [
    ...
    if any(
        ...
        raise AuditError("Sol REP-001 safe preflight output evidence is invalid")


def _prevalidate_runtime_evidence(run_root: Path) -> None:
    launch_path = run_root.resolve() / "stage5a-launch.json"
    if not launch_path.is_file():
        raise AuditError("Sol REP-001 launch evidence is missing")
    try:
        launch = json.loads(launch_path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise AuditError("Sol REP-001 launch evidence is malformed") from exc
    if not isinstance(launch, dict):
        raise AuditError("Sol REP-001 launch evidence is not an object")
    _validate_launch_preflight(launch)
    telemetry_path = run_root.resolve() / "stage5a-guidance-lineage.jsonl"
    if not telemetry_path.is_file():
        raise AuditError("Sol REP-001 telemetry evidence is missing")
    try:
        events = CORE.load_events(telemetry_path)
    except json.JSONDecodeError as exc:
        raise AuditError("Sol REP-001 telemetry evidence is malformed") from exc
    rollouts = [
    ...
    if len(set(workspace_ids)) != len(workspace_ids):
        raise AuditError("Sol REP-001 rollout workspace telemetry is duplicated")
    rollout_by_workspace: dict[str, dict[str, Any]] = {}
    for rollout in rollouts:
        if rollout.get("check_evidence_required") is not True:
            raise AuditError("Sol REP-001 rollout did not require Lean-check evidence")
        if rollout.get("check_evidence_validated") is not True:
            raise AuditError("Sol REP-001 rollout lacks validated Lean-check evidence")
        ...
        if not _is_hash(checker_hash) or not _is_hash(candidate_hash):
            raise AuditError("Sol REP-001 rollout checker or candidate hash is invalid")
        if not isinstance(checks, list):
            raise AuditError("Sol REP-001 local Lean-check evidence is not a list")
        ...
        if rollout.get("failure_before_guidance_change_sequences") != expected_sequences:
            raise AuditError("Sol REP-001 failure sequence evidence drifted")
        if rollout.get("failure_event_hashes") != expected_hashes:
            raise AuditError("Sol REP-001 failure-event hashes drifted")
    ...
    for produced in (
        ...
        if rollout is None:
            raise AuditError("Sol REP-001 produced optimizer has no source rollout")
        if produced.get("check_evidence_validated") is not True:
            raise AuditError("Sol REP-001 produced optimizer lacks validated checks")
        for key in (
            ...
            if produced.get(key) != rollout.get(key):
                raise AuditError(f"Sol REP-001 produced optimizer {key} mismatch")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Stage 5B Sol REP-001 audit error: {exc}")
        return 2
