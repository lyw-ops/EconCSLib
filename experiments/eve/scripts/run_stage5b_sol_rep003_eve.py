#!/usr/bin/env python3
"""Run pinned EvE with Sol REP-003 fail-closed liveness telemetry."""
...
def _load_terminal_contract():
    path = Path(__file__).resolve().with_name("stage5b_sol_rep003_terminal.py")
    spec = importlib.util.spec_from_file_location(
        "econcslib_stage5b_sol_rep003_terminal", path
    )
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Could not load Sol REP-003 terminal contract {path}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["econcslib_stage5b_sol_rep003_terminal"] = module
...
def derive_stream_seed(experiment_seed: int, stream_name: str) -> int:
    ...
    material = (
        f"EconCSlib-EvE-Stage5B-Sol-REP003-v1:{experiment_seed}:{stream_name}"
    ).encode()
...
def _observed_build_workspace(self) -> None:
    ...
    if self.workspace is None or self.optimizer is None:
        raise RuntimeError("Sol REP-003 workspace observation requires a built workspace")
    ...
    if not checker.is_file():
        raise RuntimeError("Sol REP-003 immutable Lean checker is missing")
...
def _load_lean_check_events(workspace: Path) -> list[dict[str, object]]:
    ...
    if not path.is_file():
        raise RuntimeError("required Sol REP-003 Lean-check evidence is missing")
    lines = [line for line in path.read_text(encoding="utf-8").splitlines() if line.strip()]
    if not lines:
        raise RuntimeError("required Sol REP-003 Lean-check evidence is empty")
    events: list[dict[str, object]] = []
    for line in lines:
        try:
            value = json.loads(line)
        except json.JSONDecodeError as exc:
            raise RuntimeError("Sol REP-003 Lean-check evidence is malformed") from exc
        if not isinstance(value, dict):
            raise RuntimeError("Sol REP-003 Lean-check event is not an object")
        events.append(value)
    return events
    ...
    *,
    ...
    for sequence, event in enumerate(events, start=1):
        if event.get("schema_version") != "2.0.0":
            raise RuntimeError("Sol REP-003 Lean-check schema is invalid")
        if event.get("sequence") != sequence:
            raise RuntimeError("Sol REP-003 Lean-check sequence is not contiguous")
        if event.get("previous_event_sha256") != previous:
            raise RuntimeError("Sol REP-003 Lean-check hash chain is invalid")
        if event.get("checker_sha256") != checker_sha256:
            raise RuntimeError("Sol REP-003 Lean check used a different checker")
        if event.get("event_sha256") != local_event_sha256(event):
            raise RuntimeError("Sol REP-003 Lean-check event hash is invalid")
        for key in (
            "candidate_sha256",
            "guidance_sha256",
            "stdout_sha256",
            ...
            if not _is_hash(event.get(key)):
                raise RuntimeError(f"Sol REP-003 Lean-check {key} is invalid")
        if event.get("python_invocation") != EXPECTED_PYTHON_INVOCATION:
            raise RuntimeError("Sol REP-003 checker invocation drifted")
        if event.get("python_executable") != EXPECTED_PYTHON_EXECUTABLE:
            raise RuntimeError("Sol REP-003 checker executable drifted")
        if event.get("python_version") != EXPECTED_PYTHON_VERSION:
            raise RuntimeError("Sol REP-003 checker Python version drifted")
        if event.get("command") != EXPECTED_CHECK_COMMAND:
            raise RuntimeError("Sol REP-003 Lean command drifted")
        exit_code = event.get("exit_code")
        failed = event.get("failed")
        if isinstance(exit_code, bool) or not isinstance(exit_code, int):
            raise RuntimeError("Sol REP-003 Lean-check exit status is invalid")
        if not isinstance(failed, bool) or failed != (exit_code != 0):
            raise RuntimeError("Sol REP-003 Lean-check failure flag is inconsistent")
        ...
        try:
            ...
            raise RuntimeError("Sol REP-003 Lean-check timestamp is malformed") from exc
        if instant.utcoffset() != dt.timedelta(0):
            raise RuntimeError("Sol REP-003 Lean-check timestamp is not UTC")
        previous = str(event["event_sha256"])
    if events[-1].get("candidate_sha256") != final_candidate_sha256:
        raise RuntimeError("Sol REP-003 final checker event does not match final candidate")
...
def _observed_run_agent(self) -> None:
    ...
    try:
        ...
        self._rep003_failure_stage = "agent-or-boundary"
        self._rep003_failure_message = str(exc)
        raise
    try:
        if self.workspace is None:
            raise TERMINAL.TerminalCheckError("Sol REP-003 terminal workspace is missing")
        ...
        if not isinstance(checker_hash, str):
            raise TERMINAL.TerminalCheckError(
                "Sol REP-003 initial checker identity is missing"
            )
        self._rep003_terminal_evidence = TERMINAL.run_terminal_checker(
            self.workspace,
            expected_checker_sha256=checker_hash,
        )
    except Exception as exc:
        self._rep003_failure_stage = "post-agent-pre-evaluation-terminal-check"
        self._rep003_failure_message = str(exc)
...
def _record_rollout_terminal(
    ...
    *,
    ...
    if getattr(self, "_rep003_terminal_recorded", False):
        return
    workspace = self.workspace
    evidence = getattr(self, "_rep003_terminal_evidence", None)
    ...
    _record(payload)
    self._rep003_terminal_recorded = True
...
def _validate_post_evaluation_snapshot(self) -> dict[str, object]:
    ...
    if not isinstance(evidence, dict) or self.workspace is None:
        raise TERMINAL.TerminalCheckError(
            "Sol REP-003 pre-evaluation terminal evidence is missing"
        )
    candidate = self.workspace / "solver" / "Candidate.lean"
    checker = self.workspace / "STAGE5A_LEAN_CHECK.py"
    if not candidate.is_file() or not checker.is_file():
        raise TERMINAL.TerminalCheckError(
            "Sol REP-003 candidate or checker disappeared during evaluation"
        )
    candidate_hash = file_sha256(candidate)
    checker_hash = file_sha256(checker)
    if candidate_hash != evidence.get("final_candidate_sha256"):
        raise TERMINAL.TerminalCheckError(
            "Sol REP-003 candidate changed after the pre-evaluation terminal check"
        )
    if checker_hash != evidence.get("lean_checker_sha256"):
        raise TERMINAL.TerminalCheckError(
            "Sol REP-003 checker changed after the pre-evaluation terminal check"
        )
    ...
    if (
        ...
        raise TERMINAL.TerminalCheckError(
            "Sol REP-003 terminal evidence changed during evaluation"
        )
    return evidence


def _observed_run_single(self, **kwargs):
    self._rep003_terminal_recorded = False
    self._rep003_terminal_evidence = None
    self._rep003_failure_stage = None
    self._rep003_failure_message = None
    try:
        result = _ORIGINAL_RUN_SINGLE(self, **kwargs)
    except Exception as exc:
        evidence = getattr(self, "_rep003_terminal_evidence", None)
        failure_stage = getattr(self, "_rep003_failure_stage", None)
        ...
        _record_rollout_terminal(
            ...
            failure_stage=failure_stage,
            failure_message=getattr(self, "_rep003_failure_message", None) or str(exc),
        )
        raise
    ...
    try:
        ...
        if self.workspace is None or self.optimizer is None:
            raise TERMINAL.TerminalCheckError(
                "Sol REP-003 rollout observation requires a workspace"
            )
        ...
        self._rep003_failure_stage = "post-evaluation-terminal-revalidation"
        self._rep003_failure_message = str(exc)
        _record_rollout_terminal(
            self,
            status="RUN_FAILED_CHECK_EVIDENCE_CONTRACT",
            check_evidence_validated=False,
            failure_stage=self._rep003_failure_stage,
...
def _seeded_from_config(cls, config, solver_evaluator, **kwargs):
    ...
    if isinstance(raw_seed, bool) or not isinstance(raw_seed, int):
        factory.close()
        raise SystemExit("Sol REP-003 requires an integer experiment_seed")
    condition = OmegaConf.to_container(
        OmegaConf.select(config, "stage5a_condition"), resolve=True
    )
    if not isinstance(condition, dict):
        factory.close()
        raise SystemExit("Sol REP-003 requires stage5a_condition metadata")
    try:
    ...
    audit = {
        ...
        "protocol_generation": "Sol REP-003",
