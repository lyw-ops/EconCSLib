#!/usr/bin/env python3
"""Run pinned EvE with Sol REP-001 fail-closed liveness telemetry."""
...
def derive_stream_seed(experiment_seed: int, stream_name: str) -> int:
    ...
    material = (
        f"EconCSlib-EvE-Stage5B-Sol-REP001-v1:{experiment_seed}:{stream_name}"
    ).encode()
...
def _observed_build_workspace(self) -> None:
    ...
    if self.workspace is None or self.optimizer is None:
        raise RuntimeError("Sol REP-001 workspace observation requires a built workspace")
    ...
    if not checker.is_file():
        raise RuntimeError("Sol REP-001 immutable Lean checker is missing")
...
def _load_lean_check_events(workspace: Path) -> list[dict[str, object]]:
    ...
    if not path.is_file():
        raise RuntimeError("required Sol REP-001 Lean-check evidence is missing")
    lines = [line for line in path.read_text(encoding="utf-8").splitlines() if line.strip()]
    if not lines:
        raise RuntimeError("required Sol REP-001 Lean-check evidence is empty")
    events: list[dict[str, object]] = []
    for line in lines:
        try:
            value = json.loads(line)
        except json.JSONDecodeError as exc:
            raise RuntimeError("Sol REP-001 Lean-check evidence is malformed") from exc
        if not isinstance(value, dict):
            raise RuntimeError("Sol REP-001 Lean-check event is not an object")
        events.append(value)
    return events
    ...
    *,
    ...
    for sequence, event in enumerate(events, start=1):
        if event.get("schema_version") != "2.0.0":
            raise RuntimeError("Sol REP-001 Lean-check schema is invalid")
        if event.get("sequence") != sequence:
            raise RuntimeError("Sol REP-001 Lean-check sequence is not contiguous")
        if event.get("previous_event_sha256") != previous:
            raise RuntimeError("Sol REP-001 Lean-check hash chain is invalid")
        if event.get("checker_sha256") != checker_sha256:
            raise RuntimeError("Sol REP-001 Lean check used a different checker")
        if event.get("event_sha256") != local_event_sha256(event):
            raise RuntimeError("Sol REP-001 Lean-check event hash is invalid")
        for key in (
            "candidate_sha256",
            "guidance_sha256",
            "stdout_sha256",
            ...
            if not _is_hash(event.get(key)):
                raise RuntimeError(f"Sol REP-001 Lean-check {key} is invalid")
        if event.get("python_invocation") != EXPECTED_PYTHON_INVOCATION:
            raise RuntimeError("Sol REP-001 checker invocation drifted")
        if event.get("python_executable") != EXPECTED_PYTHON_EXECUTABLE:
            raise RuntimeError("Sol REP-001 checker executable drifted")
        if event.get("python_version") != EXPECTED_PYTHON_VERSION:
            raise RuntimeError("Sol REP-001 checker Python version drifted")
        if event.get("command") != EXPECTED_CHECK_COMMAND:
            raise RuntimeError("Sol REP-001 Lean command drifted")
        exit_code = event.get("exit_code")
        failed = event.get("failed")
        if isinstance(exit_code, bool) or not isinstance(exit_code, int):
            raise RuntimeError("Sol REP-001 Lean-check exit status is invalid")
        if not isinstance(failed, bool) or failed != (exit_code != 0):
            raise RuntimeError("Sol REP-001 Lean-check failure flag is inconsistent")
        ...
        try:
            ...
            raise RuntimeError("Sol REP-001 Lean-check timestamp is malformed") from exc
        if instant.utcoffset() != dt.timedelta(0):
            raise RuntimeError("Sol REP-001 Lean-check timestamp is not UTC")
        previous = str(event["event_sha256"])
    if events[-1].get("candidate_sha256") != final_candidate_sha256:
        raise RuntimeError("Sol REP-001 final checker event does not match final candidate")


def _observed_run_single(self, **kwargs):
    result = _ORIGINAL_RUN_SINGLE(self, **kwargs)
    if self.workspace is None or self.optimizer is None:
        raise RuntimeError("Sol REP-001 rollout observation requires a workspace")
    candidate_path = self.workspace / "solver" / "Candidate.lean"
    if not candidate_path.is_file():
        raise RuntimeError("Sol REP-001 final candidate is missing")
    ...
    if final_checker_hash is None or final_checker_hash != initial_checker_hash:
        raise RuntimeError("Sol REP-001 immutable Lean checker changed during rollout")
...
def _seeded_from_config(cls, config, solver_evaluator, **kwargs):
    ...
    if isinstance(raw_seed, bool) or not isinstance(raw_seed, int):
        factory.close()
        raise SystemExit("Sol REP-001 requires an integer experiment_seed")
    condition = OmegaConf.to_container(
        OmegaConf.select(config, "stage5a_condition"), resolve=True
    )
    if not isinstance(condition, dict):
        factory.close()
        raise SystemExit("Sol REP-001 requires stage5a_condition metadata")
    try:
    ...
    audit = {
        ...
        "protocol_generation": "Sol REP-001",
