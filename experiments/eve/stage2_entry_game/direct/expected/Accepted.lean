...
theorem direct_nash_iff (profile : PureProfile) :
  ...
  cases challenger <;> cases incumbent <;>
    simp [IsNash, normalPayoff, play, payoff] <;> decide

theorem direct_subgamePerfect_iff (profile : PureProfile) :
    IsSubgamePerfect profile ↔ profile = (.enter, .acquiesce) := by
  rcases profile with ⟨challenger, incumbent⟩
  cases challenger <;> cases incumbent <;>
    simp [IsSubgamePerfect, IsNash, IsOptimalAfterEntry,
      normalPayoff, play, payoff] <;> decide

theorem direct_out_fight_separation :
    IsNash (.stayOut, .fight) ∧
      ¬ IsSubgamePerfect (.stayOut, .fight) := by
  constructor
  · exact (direct_nash_iff _).2 (Or.inr rfl)
  · intro h
    have hbad := h.2 .acquiesce
    norm_num [normalPayoff, play, payoff] at hbad
