"""Regressions for the frozen, zero-model-call Stage 5B Sol REP-002 protocol."""
...
RUNNER = _load("eve_stage5b_sol_rep002_test_runner", "scripts/run_stage5b_sol_rep002.py")
AUDITOR = _load("eve_stage5b_sol_rep002_test_auditor", "scripts/audit_stage5b_sol_rep002.py")
TERMINAL = _load(
    "eve_stage5b_sol_rep002_test_terminal",
    "scripts/stage5b_sol_rep002_terminal.py",
)
...
class Stage5BSolRep002ProtocolTests(unittest.TestCase):
    @classmethod
    ...
    def test_scientific_inputs_match_rep001_and_dev003_byte_for_byte(self) -> None:
        dev003 = json.loads(
            (SIDECAR_ROOT / "stage5a_dev003_protocol.json").read_text(encoding="utf-8")
        )
        for key in (
        ...
        for route in ("direct", "transport"):
            new_root = SIDECAR_ROOT / "overlay" / f"stage5b_sol_rep002_entry_game_{route}"
            old_root = SIDECAR_ROOT / "overlay" / f"stage5b_sol_rep001_entry_game_{route}"
    ...
    def test_wrapper_owns_post_agent_pre_evaluation_terminal_check(self) -> None:
        source = (SIDECAR_ROOT / "scripts/run_stage5b_sol_rep002_eve.py").read_text(
            encoding="utf-8"
        )
        observed = source[
    ...
    def test_terminal_checker_closes_post_check_candidate_mutation(self) -> None:
        with tempfile.TemporaryDirectory(prefix="sol-rep002-terminal-") as raw:
            temp = Path(raw)
            ...
            shutil.copy2(
                SIDECAR_ROOT
                / "overlay/stage5b_sol_rep002_entry_game_direct/immutable/STAGE5A_LEAN_CHECK.py",
                checker,
            )
            env = {
                **os.environ,
    ...
    def test_missing_empty_malformed_and_mismatched_checks_fail_closed(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="sol_rep002-malformed-telemetry-") as raw:
            root = Path(raw)
    ...
    def test_exact_runtime_safe_preflight_records_event_without_formal_state(self) -> None:
        with tempfile.TemporaryDirectory(prefix="sol_rep002-fake-lean-") as raw:
            temp = Path(raw)
    ...
    def test_safe_preflight_preserves_preexisting_formal_state(self) -> None:
        with tempfile.TemporaryDirectory(prefix="sol_rep002-preexisting-state-") as raw:
            temp = Path(raw)
            runs = temp / "runs"
    ...
    def test_sqlite_wal_checkpoint_does_not_change_logical_snapshot(self) -> None:
        with tempfile.TemporaryDirectory(prefix="sol-rep002-wal-") as raw:
            temp = Path(raw)
            runs = temp / "runs"
            runs.mkdir()
            ledger = temp / "ledger.sqlite3"
    ...
    def test_projected_snapshot_detects_durable_non_sqlite_change(self) -> None:
        with tempfile.TemporaryDirectory(prefix="sol-rep002-projection-") as raw:
            root = Path(raw)
    ...
    def test_fresh_sol_rep002_ledger_and_order_do_not_accept_dev002_identity(self) -> None:
        compute = self.protocol["shared_compute"]
        self.assertEqual(
            compute["fresh_run_root_parent"],
            "experiments/eve/.runtime/stage5b-sol-rep002-runs",
        )
        self.assertEqual(
            compute["durable_attempt_ledger"],
            "experiments/eve/.runtime/stage5b-sol-rep002-attempt-ledger.sqlite3",
        )
        self.assertNotIn("dev002", str(RUNNER.RUNS_ROOT).lower())
        with tempfile.TemporaryDirectory(prefix="sol_rep002-ledger-") as raw:
            temp = Path(raw)
    ...
    def test_preflight_precedes_formal_root_and_ledger_reservation(self) -> None:
        source = (SIDECAR_ROOT / "scripts/run_stage5b_sol_rep002.py").read_text(
            encoding="utf-8"
        )
    ...
    def test_protocol_stops_frozen_without_execution_authorization(self) -> None:
        ...
        self.assertIn("zero Sol REP-002 model calls", boundary)
        self.assertIn("separate explicit model/quota authorization", boundary)
        self.assertIn("Sol REP-002 execution and model calls are not authorized", boundary)
