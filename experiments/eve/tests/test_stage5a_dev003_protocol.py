"""Regressions for the frozen, zero-model-call Stage 5A DEV-003 protocol."""
...
RUNNER = _load("eve_stage5a_dev003_test_runner", "scripts/run_stage5a_dev003.py")
AUDITOR = _load("eve_stage5a_dev003_test_auditor", "scripts/audit_stage5a_dev003.py")
...
class Stage5ADev003ProtocolTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.protocol = json.loads(
            (SIDECAR_ROOT / "stage5a_dev003_protocol.json").read_text(
                encoding="utf-8"
            )
        )
    ...
    def test_protocol_identity_hash_and_frozen_assets_verify(self) -> None:
        ...
        self.assertEqual(self.protocol["version"], "3.0.0")
        self.assertEqual(self.protocol["status"], "FROZEN_NOT_YET_EXECUTED")
        RUNNER.verify_protocol_assets()

    def test_dev001_and_dev002_evidence_remain_historical(self) -> None:
        old = json.loads(
        ...
        self.assertEqual(old["id"].removesuffix("DEV-002"), self.protocol["id"].removesuffix("DEV-003"))
        ...
        self.assertEqual(
            self.protocol["supersedes_stage5a_protocol"]["protocol_id"], old["id"]
        )
    ...
    def test_missing_empty_malformed_and_mismatched_checks_fail_closed(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="dev003-malformed-telemetry-") as raw:
            root = Path(raw)
    ...
    def test_exact_runtime_safe_preflight_records_event_without_formal_state(self) -> None:
        with tempfile.TemporaryDirectory(prefix="dev003-fake-lean-") as raw:
            temp = Path(raw)
            ...
            self.assertTrue(report["formal_state_snapshot_unchanged"])
            self.assertTrue(report["dev002_state_snapshot_unchanged"])
            self.assertEqual(
                before,
    ...
    def test_safe_preflight_preserves_preexisting_formal_state(self) -> None:
        with tempfile.TemporaryDirectory(prefix="dev003-preexisting-state-") as raw:
            temp = Path(raw)
            runs = temp / "runs"
            ...
            try:
                before = (
                    RUNNER._filesystem_state_sha256(runs),
                    RUNNER._filesystem_state_sha256(ledger),
                )
                report = RUNNER.safe_preflight(
                    RUNNER.SPECS["entry-game-direct"], lean_root, toolchain
                )
    ...
    def test_fresh_dev003_ledger_and_order_do_not_accept_dev002_identity(self) -> None:
        compute = self.protocol["shared_compute"]
        self.assertEqual(
            compute["fresh_run_root_parent"],
            "experiments/eve/.runtime/stage5a-dev003-runs",
        )
        self.assertEqual(
            compute["durable_attempt_ledger"],
            "experiments/eve/.runtime/stage5a-dev003-attempt-ledger.sqlite3",
        )
        self.assertNotIn("dev002", str(RUNNER.RUNS_ROOT).lower())
        with tempfile.TemporaryDirectory(prefix="dev003-ledger-") as raw:
            temp = Path(raw)
    ...
    def test_preflight_precedes_formal_root_and_ledger_reservation(self) -> None:
        source = (SIDECAR_ROOT / "scripts/run_stage5a_dev003.py").read_text(
            encoding="utf-8"
        )
    ...
    def test_protocol_stops_frozen_without_execution_authorization(self) -> None:
        ...
        self.assertIn("zero DEV-003 model calls", boundary)
        self.assertIn("separate execution authorization", boundary)
        self.assertIn("Sol replication is not authorized", boundary)
