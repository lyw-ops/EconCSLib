...
Treat it as the normative style specification. The Chinese document is
explanatory only and must not override the English specification. When the
manual and a machine-readable rule disagree, stop and record the discrepancy
in `../../docs/research/mathlib-style/DECISIONS.md`; do not silently choose one.
