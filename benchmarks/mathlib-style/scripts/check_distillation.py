...
REQUIRED_DOC_PATHS = [
    ...
    "PHASE1_COMPLETION_REPORT_ZH.md",
    "PHASE3_COMPLETION_REPORT_ZH.md",
]
...
def check_rules_and_manuals() -> tuple[set[str], int]:
    ...
    chinese = (DOCS_ROOT / "MANUAL_ZH.md").read_text(encoding="utf-8")
    ...
    documented_zh = set(
        re.findall(r"`((?:FIL|FMT|NAM|DOC|STM|PRF|API|LOC)-[0-9]+[A-Z]?)`", chinese)
    )
    missing_zh = set(rule_ids) - documented_zh
    require(not missing_zh, f"Chinese manual omits leaf rules: {sorted(missing_zh)}")
    require("**Version:** 0.3.1" in english, "English manual version drift")
    require("**版本：** 0.3.1" in chinese, "Chinese manual version drift")
    ...
    require(
        "`schema_version` 与全部 11 个 JSON Schema 继续 byte-for-byte 冻结在 `0.3.0`"
        in compact_chinese,
        "Chinese manual does not preserve the v0.3.0 schema boundary",
    )

    manual_rows = {
        "English": parse_manual_rule_rows(english, "English"),
        "Chinese": parse_manual_rule_rows(chinese, "Chinese"),
    }
    for language, rows in manual_rows.items():
        require(set(rows) == set(rule_ids), f"{language} manual table is not exactly 75 rules")
        title_key = "en" if language == "English" else "zh"
        rule_key = "en" if language == "English" else "zh"
        for rule in rules:
            title, strength, evidence, automation, wording, validators = rows[rule["id"]]
            require(title == rule["title"][title_key], f"{language} title drift for {rule['id']}")
            require(strength == f"`{rule['rule_strength']}`", f"{language} strength drift for {rule['id']}")
            require(evidence == f"`{rule['evidence_class']}`", f"{language} evidence drift for {rule['id']}")
            require(automation == f"`{rule['automation']['level']}`", f"{language} automation drift for {rule['id']}")
            require(wording == rule["rule"][rule_key], f"{language} wording drift for {rule['id']}")
            require(
                re.findall(r"`([^`]+)`", validators)
                == rule["automation"]["validator_ids"],
                f"{language} validator list drift for {rule['id']}",
            )
    ...
    require(
        "English specification" in agent_guide
        and "must not override" in agent_guide,
        "AGENTS.md must keep the English manual normative",
    )
