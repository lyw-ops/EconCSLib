...
recommended import, growth permission, and removal policy for all 182 in-scope
...
| Structural/PMF relations | `EconCSLib.GameTheory.ExtensiveGame.Interface.Relations.Discrete` | strict morphisms/isomorphisms, information refinements, PMF trajectory relations, and weak/stuttering simulations | `Interface.Relations` additionally exposes analytic execution and the common lawful path-law realization/coupling interfaces |
| Discrete equilibrium | `EconCSLib.GameTheory.ExtensiveGame.Interface.Equilibrium.Discrete` | pure, behavioral, mixed, and countably supported general-strategy semantics; finite-fuel equilibrium, bounded complete-history-law realization, perfect recall, finite Kuhn, and uniform strategy bridges | `Interface.Equilibrium` also exposes analytic equilibrium |
...
| Objective | 32 / 37 |
| Winning | 35 / 40 |
| Stochastic winning | 50 / 57 |
| Finite execution | 33 / 40 |
| Infinite discrete execution | 38 / 45 |
| Discrete compatibility aggregate | 39 / 46 |
| Analytic execution | 58 / 66 |
| Discrete relations | 39 / 46 |
| Relations compatibility aggregate | 68 / 76 |
| Discrete equilibrium | 66 / 82 |
| Analytic equilibrium | 96 / 113 |
| Equilibrium compatibility aggregate | 101 / 118 |
| Restart | 104 / 121 |
| Discrete compilation | 87 / 105 |
| Compilation compatibility aggregate | 123 / 142 |
| Complete-stack compatibility aggregate | 132 / 152 |
...
At the 2026-08-01 audit snapshot the root has a 37-module EFG closure (165
...
source-root predicate. The established root-parameterized
...
`ObservedGame.ContinuationSemantics` is a compatibility abbreviation through
...
definitionally. `toIndexedGameFormOn` reuses the explicitly supplied root predicate and
therefore supports
`IsNashOnPresentationAt roots`; `toIndexedGameFormOn system.IsRoot` and
`IsSubgamePerfectOnAt ... system` expose equilibrium on an explicit lawful
system. `IsStandardSubgamePerfectAt ... completeSystem` additionally
certifies coverage of every structurally lawful root.

`ContinuationSemantics.isStandardSubgamePerfectAt_iff_of_surjective` is the
generic cross-game standard-SPE preservation theorem. It requires complete
lawful source and target systems, a continuation-game-form homomorphism,
utility compatibility, target strategy/deviation surjectivity, and
target-root surjectivity. These premises are intentional: a weak serializer
or declared-root map alone does not establish standard SPE. In particular,
the current FOSG weak serializer proves finite-horizon behavioral Nash
transfer but does not yet supply the lawful-root coverage,
termination-certified whole-continuation outcome compatibility, and complete
target-deviation coverage needed to instantiate this theorem. Its relation
targets macro boundaries, whereas a complete target subgame system ranges
over every lawful target history; lawful serialized micro-step roots therefore
require an additional source-coverage theorem rather than an implicit
identification.

The generic theorem's source and target are payoff-free controlled observed
games, so a logical or path-objective application does not need to invent a
dummy state-payoff type. The compatibility theorem for two
`ObservedGame`s also permits their state-payoff types to differ.
