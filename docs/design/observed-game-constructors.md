...
change. A raw `ObservedGame` literal commonly repeated all 15 structure
fields. Twelve presentations selected every complete history with the same
two-line `True` predicate and `trivial` initial proof.

The repeated presentations were not all semantically interchangeable:

| Pattern | Repeated data | Correct treatment |
|---|---|---|
| Complete-history private observation and information, direct base actions | `Observation`, `observe`, `InfoState`, `infoObserve`, `infoAt`, `InfoAction`, `actionEquiv`, and their two laws | `historyInformationPresentation` |
| Complete history also public | identity public projection | `completeInformationPresentation` |
...
| `ObservedGame.historyInformationPresentation` | complete-history private observation and information; direct `Arena.Action` fibers | public projection, designated roots |
| `ObservedGame.completeInformationPresentation` | the preceding presentation with the complete history also public | designated roots |
...
| `ObservedChanceGame.completeInformationPresentation` | complete information plus the supplied kernel | designated roots and every chance law |
...
All data-only constructors above are reducible abbreviations. If the complete
history or an action fiber already has `DecidableEq`, `Fintype`, or a
`MeasurableSpace`, the corresponding generated observation, information, or
action carrier reuses that instance definitionally. The constructors do not
add global instances or new typeclass premises to `ObservedGame`.
...
same record, including the former `Unit` public observation. The migrated
files contain compile-time projection checks. No isomorphism, cast, or
downstream strategy conversion is required.
...
- `historyInformationPresentation` uses every complete history as every
  ...
  be empty. Use a player-decision-history subtype, as
  `GameTreeOccurrenceObserved` does, when the strategy domain must exclude
  such histories.
...
- `ContinuationRootPresentation.allHistories` is presentation metadata only.
