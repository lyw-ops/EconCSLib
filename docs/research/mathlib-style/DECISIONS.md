...
## D002 — English is the sole distributed language

**Decision:** `MANUAL_EN.md` is the specification. Mathlib-style manuals,
reports, registries, and validation messages distributed by this repository
are English only.

**Reason:** One language prevents translation drift from creating incompatible
gold standards and gives agents a single stable text surface.
...
## D014 — English-only packaging migration

**Decision:** Remove the redundant translated manual, replace translated status
reports with English reports at language-neutral paths, and remove `zh` and
`*_zh` presentation fields from the four machine registries. Preserve the
English normative wording, all 75 leaf IDs, source and validator identities,
artifact version `v0.3.1`, schema version `v0.3.0`, and pilot identity.
